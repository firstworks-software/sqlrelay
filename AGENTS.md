...
- **Comment style inside functions/methods**: put comments *inside* the function/method, above the specific block they describe, rather than as one block summarizing the whole function/method from outside. A function/method header comment (on the line(s) before the signature) should be short, if present at all — explain each meaningful step in situ. This applies to new functions/methods and to adding comments to existing ones.
    - Comments go on the line(s) **before** the code they explain, not to the right of it
    - Pepper short comments through the body at the point each step happens, rather than stacking a block of comments above a block of code
    - Keep comments terse — a short phrase that names what the next line(s) do, not full sentences or restatements of the code
