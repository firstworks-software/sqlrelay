...
- **Running a query from inside a module.** `prepareQuery` / `executeQuery` on the controller each have a short overload and one taking four booleans - directives, translations, filters, triggers. The short forms disable all four. Which to use depends on what the query is. A query that stands in for the user's own gets all four enabled: upsert's converted update and splitmultiinsert's replacement single-inserts both do. A bookkeeping query gets all four disabled: savepoints' savepoint statements, globaltemptables' create statements, and replay's re-execution of a logged transaction, which must have triggers off or replay would log its own replayed queries.
- **Module state belongs to the cursor, not the module instance.** A module is instantiated once per connection, so its members are shared by every cursor on that connection. Any module that runs a query on a private cursor with triggers enabled re-enters every other module, so per-query state kept in a member will be clobbered by that nested query. Key it to the cursor instead - `src/triggers/savepoints.cpp` does this with a dictionary, and guards against a deleted cursor's entry being inherited by a later cursor at the same address by also storing the cursor's id. Note `newCursor()` cycles its temporary cursor ids rather than reusing one, which is what makes that check work.
...
- **`ChangeLog` entries**: the file is in Keep a Changelog format (https://keepachangelog.com/en/1.1.0/). A change gets an entry under the `## [Unreleased]` section at the top, in whichever of its `### Added`, `### Changed`, `### Deprecated`, `### Removed`, `### Fixed` or `### Security` subsections fits (add the subsection if it is missing, keeping that order), appended to the end of that list, not in the middle next to topically-related entries. Continuation lines are indented two spaces. Update the ChangeLog whenever something outside of `test/` changes, unless the change is already covered by an existing entry in that release (e.g. if an entry says "added some feature to c++ api", don't log further changes to that feature). Never log changes to tests. On release, `[Unreleased]` is renamed to the version and date and a fresh empty `[Unreleased]` section goes above it.
