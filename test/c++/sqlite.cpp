...
int main(int argc, char **argv) {
	...
	bool		haspragmafuncs=false;
	if (issqlite3) {
		const char	*dot=charstring::findFirst(dbversion,'.');
		int64_t		majorversion=
				charstring::convertToInteger(dbversion);
		int64_t		minorversion=(dot)?
				charstring::convertToInteger(dot+1):0;
