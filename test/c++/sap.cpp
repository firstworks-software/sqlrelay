...
int main(int argc, char **argv) {
	...
	// which isn't entirely unexpected, I'm not sure how this used to work.
	...
	//cur->setResultSetBufferSize(0);
	// we have to end the session, not just close the result set,
	// for it to be possible to drop the table
	con->endSession();
	...
	assertEquals(cur->getColumnName(0),"Database");
	// FIXME: returns 0 because there are no objects in any schema
	assertTrue(cur->rowCount()>0);
	stdoutput.printf("\n");
