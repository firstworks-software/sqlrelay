...
# instantiation
$con=SQLRelay::Connection->new("sqlrelay",9007,"/tmp/odbc-mssql.socket",
...
# every create procedure below, so it isn't usable here.  this section
# would insert fixture rows 5 through 7, which the REMAINING FIXTURE ROWS
# section below already inserts by position
...
# section would insert fixture row 8, which REMAINING FIXTURE ROWS below
...
# column length
print("COLUMN LENGTH: \n");
# odbc reports the ODBC column size - the number of characters it takes to
# display the value - where freetds reports the storage size in bytes, so
# every one of these differs from the freetds test
...
assertEquals($cur->getField(0,"testdatetime"),"2001-01-01 01:00:00.000");
assertEquals($cur->getField(0,"testsmalldatetime"),"2001-01-01 01:00:00");
...
assertEquals($cur->getField(0,"testdatetime2"),"2001-01-01 13:01:01.0000000");
print("\n");
...
assertEquals($cur->getField(7,"testdatetime"),"2008-01-01 08:00:00.000");
assertEquals($cur->getField(7,"testsmalldatetime"),"2008-01-01 08:00:00");
...
assertEquals($cur->getField(7,"testdatetime2"),"2001-01-01 13:01:01.0000000");
print("\n");
...
$secondcon=SQLRelay::Connection->new("sqlrelay",9007,"/tmp/odbc-mssql.socket",
						"testuser","testpassword",0,1);
...
# an empty (non-NULL) text column returns an empty string.  odbc reports
# a true zero length for the empty blob too, where freetds reports length
# 1 (the single 0x00 byte its encoder emits) - both compare equal to ""
assertEquals($cur->getField(0,0),"");
...
$secondcon=SQLRelay::Connection->new("sqlrelay",9033,
...
assertEquals($secondcur->getFieldLength(0,"testnchar"),4000);
...
# fails to execute.  reusing this cursor's date output bind (successfully
# populated by the execute above) across a prepareQuery/executeQuery
# pair that fails to execute, followed by another prepareQuery, used to
...
# odbc binds positionally, so there is nothing to bind by name.  even if
# there were, validateBinds() can't be used for output binds here.  When
# executing a procedure you don't declare any bind variable
...
$cur->sendQuery("drop procedure testproc");
$longoutputbindbuffer=('C' x 8000);
assertTrue($cur->sendQuery(
	"create procedure testproc \@bindval varchar(8000) output as ".
	"set \@bindval='".$longoutputbindbuffer."'"));
$cur->prepareQuery("exec testproc ?");
$cur->defineOutputBindString("1",8000);
assertTrue($cur->executeQuery());
assertEquals($cur->getOutputBindLength("1"),8000);
...
assertEquals($cur->getColumnName(0),"Database");
# odbc lists INFORMATION_SCHEMA, sys and testuser - the schemas that own
# an object - rather than every schema, so dbo isn't there
...
# the odbc module maps odbc type names, not sql server ones, so "int" and
# "datetime" both fail - INTEGER and TIMESTAMP are the names to ask for.
...
assertTrue($cur->getField(14,"column_name") eq "testdate");
assertTrue($cur->getField(15,"column_name") eq "testtime");
assertTrue($cur->getField(16,"column_name") eq "testdatetime2");
...
assertTrue($cur->getField(14,"data_type") eq "date");
assertTrue($cur->getField(15,"data_type") eq "time");
assertTrue($cur->getField(16,"data_type") eq "datetime2");
assertTrue($cur->sendQuery("drop table testtable"));
print("\n");
...
assertTrue($cur->getField(1,"table") eq "testtable");
assertEquals($cur->getField(1,"non_unique"),"0");
assertEquals($cur->getField(1,"seq_in_index"),"1");
assertTrue($cur->getField(1,"column_name") eq "col1");
