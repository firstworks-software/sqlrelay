...
class odbc_mssql extends sqlrtest {
	public static void main(String args[]) throws Exception {
		...
		if (classpath.contains("sqlrelayjdbc.jar")) {
			...
			// no prepareSQL=0 here.  the sap test needs it for a jtds
			// tds 5 prepare bug, but tds 5 is sybase-only, and this
			// test's native run is identical with and without it
			url="jdbc:jtds:sqlserver://mssql:1433;databaseName="+
							hostname;
			user="testuser";
			password="testpassword";
		}
		...
		System.out.println("    "+boolval);
		// the odbc driver reports SQL_ACCESSIBLE_TABLES=Y, and jtds
		// returns true for sql server, so both sides agree here
		assertTrue(boolval);
		System.out.println();
		...
		if (issqlrelay) {
			// the odbc module doesn't report this
		...
		if (issqlrelay) {
			...
			// it reports an empty list and every type is false
			assertFalse(boolval);
		} else {
			assertTrue(boolval);
		}
		...
		System.out.println("    "+stringval);
		if (issqlrelay) {
			assertEquals(stringval,"odbc");
		} else {
			assertEquals(stringval,"Microsoft SQL Server");
		}
		System.out.println();
		...
		System.out.println("    "+stringval);
		// varies by server version.  mssql returns a multi-line banner
		// rather than a bare version, so match against its first line
		assertContainsVersion(stringval.split("\n")[0]);
		System.out.println();
		...
		System.out.println("    "+stringval);
		if (issqlrelay) {
			// the odbc driver reports SQL_SPECIAL_CHARACTERS as
			// "#$" followed by the 62 latin-1 accented letters
			// (U+00C0 through U+00FF, less the multiplication and
			// division signs).  compare against the ascii head and
			// the total length rather than spelling the accented
			// letters out here
			assertTrue(stringval.startsWith("#$"));
			assertEquals(stringval.length(),64);
		} else {
			assertEquals(stringval,"$#@");
		}
		System.out.println();
		...
		System.out.println("    "+intval);
		if (issqlrelay) {
			// the odbc driver reports 512k
			assertEquals(intval,524288);
		} else {
		...
		System.out.println("    "+intval);
		if (issqlrelay) {
			// the odbc driver adds room for the ";group_number"
			// suffix a procedure name can carry
			assertEquals(intval,134);
		} else {
			// jtds reports the tds 7 sysname length
			assertEquals(intval,128);
		}
		...
		System.out.println("    "+intval);
		if (issqlrelay) {
			// the odbc driver reports 1 active statement, which is
			// below sql relay's maxcursors cap
		...
		System.out.println("    "+intval);
		if (issqlrelay) {
			// the odbc driver reports the join limit, not the
			// table-reference limit jtds reports
		...
		System.out.println("    "+stringval);
		// the odbc module builds these lists from the driver's
		// SQL_NUMERIC_FUNCTIONS bitmask, so they come back in the
		// module's own upper case naming rather than the lower case
		// jdbc spelling
		if (issqlrelay) {
			assertEquals(stringval,"ABS,ACOS,ASIN,ATAN,ATAN2,CEILING,COS,COT,DEGREES,EXP,FLOOR,LOG,LOG10,MOD,PI,POWER,RADIANS,RAND,ROUND,SIGN,SIN,SQRT,TAN,TRUNCATE");
		} else {
		...
		if (issqlrelay) {
			// the odbc module has no ODBC equivalent for this, so
			// the driver falls back on CLOSE_CURSORS_AT_COMMIT
		...
		System.out.println("    "+stringval);
		// the odbc driver reports SQL_OWNER_TERM as "owner", the same
		// term jtds reports for sql server
		...
		System.out.println("    "+stringval);
		if (issqlrelay) {
			// the odbc module passes the driver's SQL_KEYWORDS
			// through, and the driver reports its own list rather
			// than microsoft's documented t-sql reserved words
			assertEquals(stringval,"BACKUP,BREAK,BROWSE,BULK,CHECKPOINT,CLUSTERED,COMMITTED,COMPUTE,CONFIRM,CONTROLROW,DATABASE,DBCC,DISK,DISTRIBUTED,DUMMY,ERRLVL,ERROREXIT,EXIT,FILE,FILLFACTOR,FLOPPY,HOLDLOCK,IDENTITY_INSERT,IDENTITYCOL,IF,KILL,LINENO,MERGE,MIRROREXIT,NONCLUSTERED,OFF,OFFSETS,ONCE,OVER,PERCENT,PERM,PERMANENT,PLAN,PRINT,PROC,PROCESSEXIT,RAISERROR,READ,READTEXT,RECONFIGURE,REPEATABLE,RESTORE,RETURN,ROWCOUNT,RULE,SAVE,SERIALIZABLE,SETUSER,SHUTDOWN,STATISTICS,TAPE,TEMP,TEXTSIZE,TOP,TRAN,TRIGGER,TRUNCATE,TSEQUEL,UNCOMMITTED,UPDATETEXT,USE,WAITFOR,WHILE,WRITETEXT");
		} else {
		...
		System.out.println("    "+stringval);
		if (issqlrelay) {
			// built from the driver's SQL_STRING_FUNCTIONS bitmask
			assertEquals(stringval,"CONCAT,INSERT,LEFT,LTRIM,LENGTH,LOCATE,LCASE,REPEAT,REPLACE,RIGHT,RTRIM,SUBSTRING,UCASE,ASCII,CHAR,DIFFERENCE,LOCATE_2,SOUNDEX,SPACE,BIT_LENGTH,OCTET_LENGTH");
		} else {
		...
		System.out.println("    "+stringval);
		if (issqlrelay) {
			// built from the driver's SQL_SYSTEM_FUNCTIONS bitmask,
			// which has no flag for convert
			assertEquals(stringval,"USER,DBNAME,IFNULL");
		} else {
		...
		System.out.println("    "+stringval);
		if (issqlrelay) {
			// built from the driver's SQL_TIMEDATE_FUNCTIONS
			// bitmask, which also reports the odbc 3 functions
			assertEquals(stringval,"NOW,CURDATE,DAYOFMONTH,DAYOFWEEK,DAYOFYEAR,MONTH,QUARTER,WEEK,YEAR,CURTIME,HOUR,MINUTE,SECOND,TIMESTAMPADD,TIMESTAMPDIFF,DAYNAME,MONTHNAME,CURRENT_DATE,CURRENT_TIME,CURRENT_TIMESTAMP,EXTRACT");
		} else {
		...
		System.out.println("    "+boolval);
		if (issqlrelay) {
			// the odbc module has no ODBC equivalent for this
			assertFalse(boolval);
		} else {
			assertTrue(boolval);
		}
		...
		System.out.println("    "+boolval);
		if (issqlrelay) {
			// the driver's SQL_ALTER_TABLE bitmask reports only
			// ADD_COLUMN
			assertFalse(boolval);
		} else {
			assertTrue(boolval);
		...
		System.out.println("    "+boolval);
		if (issqlrelay) {
			// the driver's SQL_QUALIFIER_USAGE bitmask reports
			// only DML, procedure calls and table definitions
			assertFalse(boolval);
		} else {
			assertTrue(boolval);
		}
		...
		System.out.println("    "+boolval);
		if (issqlrelay) {
			// the driver reports SQL_GB_GROUP_BY_CONTAINS_SELECT,
			// which is short of SQL_GB_NO_RELATION
			assertFalse(boolval);
		} else {
			assertTrue(boolval);
		}
		...
		System.out.println("    "+boolval);
		if (issqlrelay) {
			// the driver reports SQL_INTEGRITY=Y
			assertTrue(boolval);
		} else {
			assertFalse(boolval);
		}
		...
		System.out.println("    "+boolval);
		if (issqlrelay) {
			// the odbc module has no ODBC equivalent for this, and
			// odbc binds positionally anyway
			assertFalse(boolval);
		} else {
			assertTrue(boolval);
		}
		...
		System.out.println("    "+boolval);
		if (issqlrelay) {
			// the driver reports SQL_CB_DELETE for both the commit
			// and rollback cursor behaviors, not SQL_CB_PRESERVE
			assertFalse(boolval);
		} else {
			assertTrue(boolval);
		}
		...
		System.out.println("    "+boolval);
		// the driver reports SQL_CB_DELETE, and jtds reports this for
		// sybase only, so both sides agree here
		assertFalse(boolval);
		System.out.println();
		...
		System.out.println("    "+boolval);
		if (issqlrelay) {
			// same SQL_CB_DELETE cursor behavior as above
			assertFalse(boolval);
		} else {
			assertTrue(boolval);
		}
		...
		System.out.println("    "+boolval);
		if (issqlrelay) {
			// the odbc module has no ODBC equivalent for the
			// result set concurrency list, so it reports an empty
			// list and every combination is false
			assertFalse(boolval);
		} else {
			assertTrue(boolval);
		}
		...
		if (issqlrelay) {
			// the odbc module has no ODBC equivalent for the
			// result set type list, so it reports an empty list
			// and every type is false
			assertFalse(boolval);
		} else {
			assertTrue(boolval);
		}
		...
		// the odbc module reports no affected row count for ddl, which
		// the driver hands back as SUCCESS_NO_INFO
		...
		assertEquals(rsmd.getColumnName(18),"testdatetime2");
		System.out.println();
		...
		// #4780 - the date/time family columns, appended after
		// testurl.  sqlrelay reports the type it maps them onto.
		// jtds 1.3.1 predates all three mssql types and hands them
		// back as nvarchar, rendered as text
		if (issqlrelay) {
			assertEquals(rsmd.getColumnTypeName(16),"DATE");
		} else {
			assertEquals(rsmd.getColumnTypeName(16),"nvarchar");
		}
		if (issqlrelay) {
			assertEquals(rsmd.getColumnTypeName(17),"TIME");
		} else {
			assertEquals(rsmd.getColumnTypeName(17),"nvarchar");
		}
		if (issqlrelay) {
			assertEquals(rsmd.getColumnTypeName(18),"TIMESTAMP");
		} else {
			assertEquals(rsmd.getColumnTypeName(18),"nvarchar");
		}
		System.out.println();
		...
		// for a number column the driver reports the column's
		// precision, and the odbc module reports the driver's
		// SQL_DESC_PRECISION rather than the 0 freetds reports.  for
		// an approximate numeric that is the mantissa bit count, so
		// real is 24 and float is 53
		assertEquals(rsmd.getPrecision(1),10);
		assertEquals(rsmd.getPrecision(2),5);
		assertEquals(rsmd.getPrecision(3),3);
		if (issqlrelay) {
			assertEquals(rsmd.getPrecision(4),24);
		} else {
			assertEquals(rsmd.getPrecision(4),7);
		}
		if (issqlrelay) {
			assertEquals(rsmd.getPrecision(5),53);
		} else {
		...
		if (issqlrelay) {
			...
			assertEquals(rsmd.getPrecision(17),16);
		}
		if (issqlrelay) {
			assertEquals(rsmd.getPrecision(18),7);
		} else {
			assertEquals(rsmd.getPrecision(18),27);
		}
		System.out.println();
		...
		for (int i=1; i<=4; i++) {
			...
			// after testurl.  the odbc module renders these
			// straight from the driver's SQL_TIMESTAMP_STRUCT
			// into fixed ISO forms, which happen to match what
			// the jtds reference driver renders, so no
			// issqlrelay branch is needed on the values
			...
			assertEquals(rs.getString(17),"13:01:01.0000000");
			...
			// datetime2
			System.out.println("  row "+i+" - datetime2");
		...
		for (int i=1; i<=4; i++) {
			...
			// date
			System.out.println("  row "+i+" - date");
			assertEquals(rs.getString("testdate"),"2001-01-01");
			...
			// datetime2
			System.out.println("  row "+i+" - datetime2");
			assertEquals(rs.getString("testdatetime2"),
					"2001-01-01 13:01:01.0000000");
			tsvar=rs.getTimestamp("testdatetime2");
			cal.setTime(tsvar);
			assertEquals(cal.get(Calendar.YEAR),2001);
			assertEquals(cal.get(Calendar.MONTH),Calendar.JANUARY);
			assertEquals(cal.get(Calendar.DAY_OF_MONTH),1);
		...
		//
		// at read committed, a plain count(*) scan blocks on the
		// writer's uncommitted rows until the transaction ends, so the
		// test would hang rather than fail.  readpast skips the locked
		// rows instead, which still counts only committed rows and so
		// still catches a premature commit.  it does assume the
		// writer's locks stay at row granularity; were they to
		// escalate, committed rows would be skipped too and the counts
		// would come back low
		...
		// unlike the freetds module, the odbc module supports callable
		// statements with output parameters
		cstmt.execute();
		...
		// the odbc module lists only the schemas that own an object -
		// INFORMATION_SCHEMA, sys and testuser - so dbo isn't there
		found=false;
		while (rs.next()) {
		...
		assertTrue(rs.next());
		// the odbc module asks the driver for the table type list, and
		// the driver reports the same SYSTEM TABLE, TABLE, VIEW that
		// the native driver does
		...
		if (issqlrelay) {
			...
			// AbstractMethodError - mssql puts the table in the
			// creating user's schema, so compare against the login
			assertEquals(rs.getString("TABLE_SCHEM"),user);
		}
		...
		// underlying mssql type names here, so no issqlrelay branch
		assertTrue(rs.next());
		...
		assertTrue(rs.next());
		assertEquals(rs.getString("COLUMN_NAME"),"testdatetime2");
		assertEquals(rs.getString("TYPE_NAME"),"datetime2");
		rs.close();
		con.setAutoCommit(true);
		stmt.executeUpdate("drop table testtable");
		con.setAutoCommit(false);
		...
		while (rs.next()) {
			...
			// the odbc module reports the name the driver gives it,
			// which carries the ";group_number" suffix
			String suffix=(issqlrelay)?";1":"";
			if (name.equals("testproc1"+suffix) ||
					name.equals("testproc2"+suffix) ||
					name.equals("testproc3"+suffix) ||
					name.equals("testproc4"+suffix)) {
				// both the odbc module and native jTDS report
				// the type as procedureReturnsResult
				assertEquals(
