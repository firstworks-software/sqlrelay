...
class freetds_mssql extends sqlrtest {
	public static void main(String args[]) throws Exception {
		...
		if (issqlrelay) {
			...
			// jtds returns true for sql server
			assertTrue(boolval);
		}
		System.out.println();
		...
		if (issqlrelay) {
			...
			// jtds reports this for sybase only
			assertFalse(boolval);
		}
		System.out.println();
		...
		// test/erlang/freetds-mssql.erl - #9389 was that it came
		// back as a full maxfieldsize buffer of NUL bytes
		assertEquals(rs.getString(1),"");
		assertFalse(rs.wasNull());
		assertEquals(rs.getString(2),null);
		assertTrue(rs.wasNull());
		// freetds encodes an empty image as a single 0x00 byte,
		// the native driver returns a true zero-length array
		...
		assertTrue(rs.next());
		if (issqlrelay) {
			// sql relay's mssql table type list is
			// ALIAS, SYNONYM, TABLE, VIEW
