...
int main(int argc, char **argv) {
	...
	*dot='\0';
	...
	if (issqlrelay) {
		...
		// MariaDB's ODBC driver always returns SQL_AUTOCOMMIT_ON
	...
	if (issqlrelay) {
		...
		// MariaDB's ODBC driver always returns SQL_MODE_READ_WRITE
		// regardless of what was set.
	...
	if (issqlrelay) {
		// substitutes with SQL_ASYNC_ENABLE_OFF,
		// returns SQL_SUCCESS_WITH_INFO,
	...
	if (issqlrelay) {
		...
		// MySQL ODBC reports the connected server name; just
		// verify non-empty
	...
	if (issqlrelay) {
		...
		// MySQL identifier case sensitivity depends on the
		// underlying filesystem (lower_case_table_names); the
		// driver typically reports SQL_IC_MIXED on linux when
	...
	if (issqlrelay) {
		...
		// native MySQL ODBC reports the backtick as the quote
		// character (the default; ANSI mode would report ")
		assertEqualDbc(dbc,(const char *)strval,"`");
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	...
	erg=SQLGetInfo(dbc,SQL_TXN_ISOLATION_OPTION,
			...
			&vallen);
	if (issqlrelay) {
		// TODO: sqlrelay's reported value is the union of what the
		// backend supports; for MySQL all four levels are valid
		assertEqualDbc(dbc,(int)uintval,
	...
	if (issqlrelay) {
		...
		// TODO: native MySQL ODBC driver filename; the iterative
		// debug pass should pin this to the actual reported value
		// (libmyodbc8w.so, libmyodbc8a.so, etc.). For now just
		// require non-empty.
		assertTrueDbc(dbc,vallen>0);
	}
	assertSuccessDbc(dbc,erg);
	...
	if (issqlrelay) {
		...
		// TODO: native MySQL ODBC version string varies; pin
		// during iterative debug. For now just require non-empty.
		assertTrueDbc(dbc,vallen>0);
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	...
	erg=SQLGetInfo(dbc,SQL_CONVERT_GUID,
			...
			(SQLSMALLINT)sizeof(uintval),&vallen);
	if (issqlrelay) {
		// SQL Relay returns HYC00 "Optional feature not
		// implemented".
		assertFailureDbc(dbc,erg);
	} else {
		// MariaDB returns 0.
	...
	erg=SQLGetInfo(dbc,SQL_SQL92_ROW_VALUE_CONSTRUCTOR,
			(SQLPOINTER)&uintval,
			(SQLSMALLINT)sizeof(uintval),&vallen);
	// Both drivers report VALUE_EXPRESSION|NULL|DEFAULT|
	// ROW_SUBQUERY.
	assertEqualDbc(dbc,(int)uintval,
	...
	if (issqlrelay) {
		...
		// MariaDB's ODBC driver does not implement this
		// infotype.
		assertFailureDbc(dbc,erg);
	}
	stdoutput.printf("\n");
	...
	// MySQL (with InnoDB) supports all four standard isolation levels;
	// each set should succeed.
	erg=SQLSetConnectAttr(dbc,SQL_ATTR_TXN_ISOLATION,
			(SQLPOINTER)(uintptr_t)SQL_TXN_READ_UNCOMMITTED,0);
	assertSuccessDbc(dbc,erg);
	...
	if (issqlrelay) {
		// SQL Relay only supports SQL_NONSCROLLABLE;
		// substitutes with SQL_NONSCROLLABLE and returns
		// SQL_SUCCESS_WITH_INFO, SQLSTATE 01S02
	...
	if (issqlrelay) {
		...
		// MariaDB promotes the cursor type to SQL_CURSOR_STATIC
		// when SQL_SCROLLABLE is requested and reports that
		// value (3) here instead of SQL_SCROLLABLE (1).
	...
	// SQL_INSENSITIVE
	// SQL Relay only supports SQL_INSENSITIVE and substitutes
	// other values; MariaDB only supports SQL_UNSPECIFIED and
	// substitutes other values.  Both return
	// SQL_SUCCESS_WITH_INFO + SQLSTATE 01S02 when substituting,
	// SQL_SUCCESS when the requested value matches.
	erg=SQLSetStmtAttr(stmt,SQL_ATTR_CURSOR_SENSITIVITY,
	...
	// Both drivers substitute and return
	// SQL_SUCCESS_WITH_INFO + SQLSTATE 01S02 ("Option value
	// changed").
	assertEqualStmt(stmt,(int)erg,(int)SQL_SUCCESS_WITH_INFO);
	erg=SQLGetStmtAttr(stmt,SQL_ATTR_CURSOR_SENSITIVITY,
			(SQLPOINTER)&stmtulenval,0,&stmtstrlen);
	...
	// SQL_SC_TRY_UNIQUE (sqlrelay doesn't implement positioned
	// updates, so it can't make uniqueness guarantees)
	erg=SQLSetStmtAttr(stmt,SQL_ATTR_SIMULATE_CURSOR,
			(SQLPOINTER)(uintptr_t)SQL_SC_TRY_UNIQUE,0);
	if (issqlrelay) {
		// substitutes with SQL_SC_NON_UNIQUE,
		// returns SQL_SUCCESS_WITH_INFO,
		// SQLSTATE 01S02 ("Option value changed")
		assertEqualStmt(stmt,(int)erg,(int)SQL_SUCCESS_WITH_INFO);
	} else {
	...
	if (issqlrelay) {
		...
		// MariaDB always returns SQL_RD_ON regardless of what
		// was set.
	...
	// behavior isn't gated on it.  MariaDB accepts get but
	// rejects every set with HY092 "Invalid attribute identifier".
	...
	// restore the original (NULL on both drivers; non-NULL is
	// rejected with HYC00 by either, which is harmless here).
	erg=SQLSetStmtAttr(stmt,SQL_ATTR_FETCH_BOOKMARK_PTR,
			(SQLPOINTER)stmtptrinit,SQL_IS_POINTER);
	if (issqlrelay) {
		...
		// MariaDB rejects with HYC00 even though stmtptrinit
		// is NULL.
		assertFailureStmt(stmt,erg);
	}
	stdoutput.printf("\n");
	...
	intval=2;
	bigintval=2;
	floatval=(SQLREAL)2.1;
	realval=(SQLDOUBLE)2.1;
	decimalval=(SQLCHAR *)"2.1";
	dateval.year=2002;
	dateval.month=1;
	dateval.day=1;
	timeval.hour=2;
	...
	intval=3;
	bigintval=3;
	floatval=(SQLREAL)3.1;
	realval=(SQLDOUBLE)3.1;
	decimalval=(SQLCHAR *)"3.1";
	dateval.year=2003;
	dateval.month=1;
	dateval.day=1;
	timeval.hour=3;
	...
	intval=4;
	bigintval=4;
	floatval=(SQLREAL)4.1;
	realval=(SQLDOUBLE)4.1;
	decimalval=(SQLCHAR *)"4.1";
	dateval.year=2004;
	dateval.month=1;
	dateval.day=1;
	timeval.hour=4;
	...
	erg=SQLBindParameter(stmt,17,SQL_PARAM_INPUT,
			...
			0,&mediumtextlen);
	assertSuccessStmt(stmt,erg);
	// longtextval here is the application token returned by SQLParamData;
	// the real value is supplied via SQLPutData below
	erg=SQLBindParameter(stmt,18,SQL_PARAM_INPUT,
			SQL_C_CHAR,SQL_LONGVARCHAR,
			0,0,(SQLPOINTER)longtextval,
			0,&longtextdaelen);
	...
	if (issqlrelay) {
		...
		// MariaDB describes real as SQL_DOUBLE/15/0 (REAL is
		// a synonym for DOUBLE PRECISION).
	...
	if (issqlrelay) {
		assertEqualStmt(stmt,(int)colsize,6);
	} else {
		// MariaDB reports the precision (2) instead of the
		// display width (6).
	...
	if (issqlrelay) {
		...
		// MariaDB reports the character count (40) instead of
		// the utf8mb4 byte width (160).
	...
	if (issqlrelay) {
		...
		// BLOB: MariaDB reports SQL_LONGVARBINARY (-4) instead
		// of SQL_VARBINARY (-2).
	...
	// honored.  MariaDB embeds the catalog/schema as a
	// non-quoted identifier when metadata_id is true.
	...
	// type info list
	// MySQL: walk the result set looking for INT, VARCHAR, CHAR, DATE,
	// TEXT, BLOB
	...
	assertSuccessStmt(stmt,erg);
	SQLEndTran(SQL_HANDLE_DBC,dbc,SQL_COMMIT);
	// MariaDB raises "Unknown column" if the catalog parameter
	// is non-NULL, so pass NULL for native and the empty string
	// for SQL Relay.  Also force metadata_id off so the table
	// name is treated as a pattern.
	...
	// per test/c++/mysql.cpp the MySQL types reported are uppercase
	// INT/CHAR/VARCHAR/DATE and parameter mode is 1 (SQL_PARAM_INPUT)
