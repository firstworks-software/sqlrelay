...
int main(int argc, char **argv) {
	...
	*dot='\0';
	...
	if (issqlrelay) {
		incstr.append(
			"Driver={SQL Relay};"
			"Server=sqlrelay;Port=9006;"
			"Socket=/tmp/saptest.socket;"
	...
	erg=SQLSetConnectAttr(dbc,SQL_ATTR_TRACEFILE,
			(SQLPOINTER)"/tmp/odbctrace-sap.log",SQL_NTS);
	assertSuccessDbc(dbc,erg);
	erg=SQLGetConnectAttr(dbc,SQL_ATTR_TRACEFILE,
			(SQLPOINTER)dbcstrval,sizeof(dbcstrval),&dbcstrlen);
	assertSuccessDbc(dbc,erg);
	assertEqualDbc(dbc,(const char *)dbcstrval,"/tmp/odbctrace-sap.log");
	stdoutput.printf("\n");
	...
	// the native sap driver doesn't implement this ODBC 3.8 infotype
	...
	// the native sap driver doesn't implement this infotype
	#if (ODBCVER >= 0x0380)
	...
	if (issqlrelay) {
		...
		// the native sap driver reports SQL_RD_OFF before any set
		assertEqualStmt(stmt,(int)stmtinitial,(int)SQL_RD_OFF);
	}
	...
	if (issqlrelay) {
		...
		// ...and SQL_RD_ON after any set, regardless of the value
	...
	for (;;) {
		...
		if (!charstring::compareIgnoringCase(
			...
			// native SAP driver reports the ODBC 2.x date/time
			// code; SQL Relay reports the ODBC 3.x code
			if (issqlrelay) {
