...
// freetds-mssql.cpp needs an assertSuccessOrNoDataStmt() helper because the
// native FreeTDS ODBC driver returns SQL_NO_DATA from DDL statements
// (create/drop table, create/drop procedure) even though they succeed.  The
// native arm here is msodbcsql18, which returns SQL_SUCCESS from DDL, the
// same as SQL Relay, so plain assertSuccessStmt() does for both arms -
// measured, every DDL site in this file returns 0 through msodbcsql18.
...
int main(int argc, char **argv) {
	...
	*dot='\0';
	...
	if (issqlrelay) {
		...
		// the mssql database is named after the host this test runs
		// on.
		//
		// this native arm goes through msodbcsql18, the same driver
		// the odbc-mssql instance's mssqlodbc dsn uses, rather than
		// through FreeTDS the way freetds-mssql.cpp's native arm
		// does.  TrustServerCertificate=yes is required because
		// msodbcsql18 defaults Encrypt=yes and the server's
		// certificate isn't trusted here.  test.sh never runs the
		// native arm; it's hand-run
		incstr.append(
	...
	erg=SQLGetConnectAttr(dbc,SQL_ATTR_PACKET_SIZE,
			(SQLPOINTER)&dbcuintval,0,&dbcstrlen);
	assertSuccessDbc(dbc,erg);
	// both arms honor the 2048 requested pre-connect
	...
	if (issqlrelay) {
		...
		// msodbcsql18 implements SQL_ATTR_METADATA_ID fully - the
		// value the app sets is the value that comes back, with no
		// substitution (FreeTDS raises HY092 for both get and set)
		erg=SQLGetConnectAttr(dbc,SQL_ATTR_METADATA_ID,
				(SQLPOINTER)&dbcinitial,0,&dbcstrlen);
		assertSuccessDbc(dbc,erg);
		assertEqualDbc(dbc,(int)dbcinitial,(int)SQL_FALSE);
	...
	if (issqlrelay) {
		...
		// msodbcsql18 does support statement-level async (it reports
		// SQL_AM_STATEMENT for SQL_ASYNC_MODE), so SQL_ASYNC_ENABLE_ON
		// is accepted and comes back unchanged, with no substitution
		// (FreeTDS raises HY092 for both get and set)
		erg=SQLGetConnectAttr(dbc,SQL_ATTR_ASYNC_ENABLE,
				(SQLPOINTER)&dbcinitial,0,&dbcstrlen);
		assertSuccessDbc(dbc,erg);
		assertEqualDbc(dbc,(int)dbcinitial,(int)SQL_ASYNC_ENABLE_OFF);
	...
	if (issqlrelay) {
		...
		// msodbcsql18 does auto-populate the IPD from SQLPrepare
		// (FreeTDS raises HY092 here)
		erg=SQLGetConnectAttr(dbc,SQL_ATTR_AUTO_IPD,
				(SQLPOINTER)&dbcuintval,0,&dbcstrlen);
		assertSuccessDbc(dbc,erg);
		assertEqualDbc(dbc,(int)dbcuintval,(int)SQL_TRUE);
	...
	if (issqlrelay) {
		...
		// msodbcsql18 doesn't implement SQL_ATTR_QUIET_MODE; get and
		// set both raise HY092 (FreeTDS implements it)
		erg=SQLGetConnectAttr(dbc,SQL_ATTR_QUIET_MODE,
				(SQLPOINTER)&dbcptrinit,0,&dbcstrlen);
		assertFailureDbc(dbc,erg);
		erg=SQLSetConnectAttr(dbc,SQL_ATTR_QUIET_MODE,
	...
	// both arms report an empty initial translate lib
	...
	// both arms implement this and report 0
	...
	if (issqlrelay) {
		...
		// msodbcsql18 answers the get - connection-level async is
		// off - but rejects the set with HY092, even though
		// SQL_ASYNC_DBC_FUNCTIONS claims SQL_ASYNC_DBC_CAPABLE
		erg=SQLGetConnectAttr(dbc,
				SQL_ATTR_ASYNC_DBC_FUNCTIONS_ENABLE,
				(SQLPOINTER)&dbcinitial,0,&dbcstrlen);
		assertSuccessDbc(dbc,erg);
		assertEqualDbc(dbc,(int)dbcinitial,
				(int)SQL_ASYNC_DBC_ENABLE_OFF);
		erg=SQLSetConnectAttr(dbc,
				SQL_ATTR_ASYNC_DBC_FUNCTIONS_ENABLE,
				(SQLPOINTER)(uintptr_t)
	...
	erg=SQLGetInfo(dbc,SQL_MAX_CONCURRENT_ACTIVITIES,
			(SQLPOINTER)&usmallintval,
			(SQLSMALLINT)sizeof(usmallintval),&vallen);
	// the odbc module passes the driver's max_statements straight
	// through, and the ms sql server driver reports 1
	...
	erg=SQLGetInfo(dbc,SQL_FETCH_DIRECTION,
			...
			&vallen);
	if (issqlrelay) {
		// the odbc module reports no fetch directions at all
		assertEqualDbc(dbc,(int)uintval,0);
	} else {
		// the native driver reports all six fetch directions plus
		// SQL_FD_FETCH_BOOKMARK
		assertEqualDbc(dbc,(int)uintval,191);
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	...
	erg=SQLGetInfo(dbc,SQL_SEARCH_PATTERN_ESCAPE,
			(SQLPOINTER)strval,(SQLSMALLINT)sizeof(strval),
			&vallen);
	// both drivers report backslash as the search
	...
	if (issqlrelay) {
		...
		// SQL Relay reports the name of the connection module, the
		// native driver reports what the database calls itself
		assertEqualDbc(dbc,(const char *)strval,
	...
	erg=SQLGetInfo(dbc,SQL_ACCESSIBLE_TABLES,
			(SQLPOINTER)strval,(SQLSMALLINT)sizeof(strval),
			&vallen);
	// the odbc module passes through what the driver reports, and both
	// the ms sql server driver and the native freetds driver wrongly
	// report that every table is accessible; jdbc
	// allTablesAreSelectable() correctly returns false
	...
	erg=SQLGetInfo(dbc,SQL_USER_NAME,
			(SQLPOINTER)strval,(SQLSMALLINT)sizeof(strval),
			&vallen);
	// both arms report the login name (suser_name())
	...
	if (issqlrelay) {
		...
		// msodbcsql18 adds its own SQL_TXN_SS_SNAPSHOT bit (32)
		assertEqualDbc(dbc,(int)uintval,47);
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	...
	if (issqlrelay) {
		...
		assertEqualDbc(dbc,(int)uintval,
			(int)(SQL_GD_BLOCK|SQL_GD_OUTPUT_PARAMS));
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	...
	erg=SQLGetInfo(dbc,SQL_ALTER_TABLE,
			...
			&vallen);
	if (issqlrelay) {
		// the odbc module reports only the add column flags - the
		// ms sql server driver doesn't report drop column
		assertEqualDbc(dbc,(int)uintval,
	...
	if (issqlrelay) {
		...
		// msodbcsql18 reports "#$" plus the Latin-1 letters
		// 0xc0-0xff, less the multiplication (0xd7) and division
		// (0xf7) signs
		stringbuffer	specialchars;
		specialchars.append("#$");
		for (int ch=0xc0; ch<=0xff; ch++) {
			if (ch!=0xd7 && ch!=0xf7) {
				specialchars.append((char)ch);
			}
		}
		assertEqualDbc(dbc,(const char *)strval,
					specialchars.getString());
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	...
	if (issqlrelay) {
		...
		assertEqualDbc(dbc,(int)usmallintval,4096);
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	...
	if (issqlrelay) {
		...
		// msodbcsql18 reports 128 times the network packet size,
		// and SQL_ATTR_PACKET_SIZE was set to 2048 pre-connect
		assertEqualDbc(dbc,(int)uintval,262144);
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	...
	erg=SQLGetInfo(dbc,SQL_CURSOR_SENSITIVITY,
			...
			&vallen);
	if (issqlrelay) {
		// the ms sql server driver leaves it unspecified
	...
	erg=SQLGetInfo(dbc,SQL_DESCRIBE_PARAMETER,
			(SQLPOINTER)strval,(SQLSMALLINT)sizeof(strval),
			&vallen);
	// the ms sql server driver can describe parameters
	assertEqualDbc(dbc,(const char *)strval,"Y");
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	#endif
	...
	erg=SQLGetInfo(dbc,SQL_COLLATION_SEQ,
			(SQLPOINTER)strval,(SQLSMALLINT)sizeof(strval),
			&vallen);
	assertEqualDbc(dbc,(const char *)strval,"ISO 8859-1");
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	#endif
	...
	if (issqlrelay) {
		...
		// msodbcsql18 reports F for this deprecated ODBC 1.0
		// selector, even though mssql does support outer joins;
		// SQL_OJ_CAPABILITIES below is what it actually answers
		assertEqualDbc(dbc,(const char *)strval,"F");
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	...
	if (issqlrelay) {
		// the ms sql server driver reports no scroll options
		// through the odbc module
		assertEqualDbc(dbc,(int)uintval,0);
	} else {
		assertEqualDbc(dbc,(int)uintval,
	...
	erg=SQLGetInfo(dbc,SQL_NUMERIC_FUNCTIONS,
			(SQLPOINTER)&uintval,
			(SQLSMALLINT)sizeof(uintval),&vallen);
	// both arms report every numeric function, TRUNCATE included
	assertEqualDbc(dbc,(int)uintval,
		(int)(SQL_FN_NUM_TRUNCATE|
			SQL_FN_NUM_ABS|
			SQL_FN_NUM_ACOS|
			SQL_FN_NUM_ASIN|
			SQL_FN_NUM_ATAN|
	...
	erg=SQLGetInfo(dbc,SQL_STRING_FUNCTIONS,
			(SQLPOINTER)&uintval,
			(SQLSMALLINT)sizeof(uintval),&vallen);
	// both arms report the same set, LOCATE_2, BIT_LENGTH and
	// OCTET_LENGTH included
	assertEqualDbc(dbc,(int)uintval,
	...
	if (issqlrelay) {
		...
		assertEqualDbc(dbc,(int)uintval,10517887);
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	...
	if (issqlrelay) {
		...
		assertEqualDbc(dbc,(int)uintval,10775839);
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	...
	if (issqlrelay) {
		...
		assertEqualDbc(dbc,(int)uintval,31883135);
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	...
	erg=SQLGetInfo(dbc,SQL_CONVERT_DATE,
			(SQLPOINTER)&uintval,
			(SQLSMALLINT)sizeof(uintval),&vallen);
	// both arms report no conversions from this type
	assertEqualDbc(dbc,(int)uintval,0);
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	...
	if (issqlrelay) {
		...
		assertEqualDbc(dbc,(int)uintval,10514815);
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	...
	erg=SQLGetInfo(dbc,SQL_CONVERT_GUID,
			...
			(SQLSMALLINT)sizeof(uintval),&vallen);
	if (issqlrelay) {
		// sqlrelay doesn't support this selector; returns HYC00
		assertFailureDbc(dbc,erg);
	} else {
		assertEqualDbc(dbc,(int)uintval,27263233);
		assertSuccessDbc(dbc,erg);
	}
	stdoutput.printf("\n");
	...
	if (issqlrelay) {
		...
		// every SQLSetPos operation
		assertEqualDbc(dbc,(int)uintval,31);
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	...
	if (issqlrelay) {
		...
		// positioned delete and update, plus select for update
		assertEqualDbc(dbc,(int)uintval,
			(int)(SQL_PS_POSITIONED_DELETE|
				SQL_PS_POSITIONED_UPDATE|
	...
	if (issqlrelay) {
		...
		// the app's own inserts and updates are visible
		assertEqualDbc(dbc,(int)uintval,
			(int)(SQL_SS_ADDITIONS|SQL_SS_UPDATES));
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	...
	if (issqlrelay) {
		// sqlrelay returns
		// SQL_GB_GROUP_BY_CONTAINS_SELECT|SQL_GB_COLLATE, not a
	...
	erg=SQLGetInfo(dbc,SQL_MAX_CHAR_LITERAL_LEN,
			...
			(SQLSMALLINT)sizeof(uintval),&vallen);
	if (issqlrelay) {
		assertEqualDbc(dbc,(int)uintval,524288);
	} else {
		// like SQL_MAX_STATEMENT_LEN, 128 times the packet size
		assertEqualDbc(dbc,(int)uintval,262144);
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	...
	erg=SQLGetInfo(dbc,SQL_TIMEDATE_ADD_INTERVALS,
			(SQLPOINTER)&uintval,
			(SQLSMALLINT)sizeof(uintval),&vallen);
	// mssql supports all nine intervals, and both arms report them
	...
	if (issqlrelay) {
		...
		// scrolled fetch, bookmarks, the SQLSetPos operations,
		// positioned update/delete and bulk operations
		assertEqualDbc(dbc,(int)uintval,130631);
	}
	stdoutput.printf("\n");
	#endif
	...
	assertSuccessDbc(dbc,erg);
	if (issqlrelay) {
		// sqlrelay has no dynamic cursors
		assertEqualDbc(dbc,(int)uintval,0);
	} else {
		assertEqualDbc(dbc,(int)uintval,69599);
	}
	stdoutput.printf("\n");
	#endif
	...
	if (issqlrelay) {
		...
		// fetch next, bookmark, positioned update, delete and
		// refresh, and bulk operations
		assertEqualDbc(dbc,(int)uintval,57345);
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	#endif
	...
	if (issqlrelay) {
		...
		assertEqualDbc(dbc,(int)uintval,3983);
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	#endif
	...
	erg=SQLGetInfo(dbc,SQL_INFO_SCHEMA_VIEWS,
			(SQLPOINTER)&uintval,
			(SQLSMALLINT)sizeof(uintval),&vallen);
	// every information_schema view ms sql server has, which is all of
	// them but ASSERTIONS, CHARACTER_SETS, COLLATIONS, SQL_LANGUAGES,
	// TRANSLATIONS and USAGE_PRIVILEGES - both arms
	assertEqualDbc(dbc,(int)uintval,
	...
	erg=SQLGetInfo(dbc,SQL_KEYSET_CURSOR_ATTRIBUTES1,
			...
			(SQLSMALLINT)sizeof(uintval),&vallen);
	if (issqlrelay) {
		// sqlrelay has no keyset cursors
		assertEqualDbc(dbc,(int)uintval,0);
	} else {
		assertEqualDbc(dbc,(int)uintval,1048143);
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	#endif
	...
	if (issqlrelay) {
		...
		assertEqualDbc(dbc,(int)uintval,73695);
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	#endif
	...
	if (issqlrelay) {
		...
		assertEqualDbc(dbc,(int)uintval,(int)SQL_OIC_LEVEL2);
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	#endif
	...
	if (issqlrelay) {
		...
		// everything but QUANTIFIED_COMPARISON, plus MATCH_UNIQUE_FULL
	...
	if (issqlrelay) {
		...
		// scrolled fetch, bookmarks, SQLSetPos positioning and
		// bulk operations
		assertEqualDbc(dbc,(int)uintval,528975);
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	#endif
	...
	erg=SQLGetInfo(dbc,SQL_AGGREGATE_FUNCTIONS,
			...
			(SQLSMALLINT)sizeof(uintval),&vallen);
	assertSuccessDbc(dbc,erg);
	// both arms report only SQL_AF_ALL
	...
	if (issqlrelay) {
		...
		// msodbcsql18 claims it here even though it rejects
		// SQL_ATTR_ASYNC_DBC_FUNCTIONS_ENABLE with HY092
		assertEqualDbc(dbc,(int)uintval,(int)SQL_ASYNC_DBC_CAPABLE);
	}
	stdoutput.printf("\n");
	#endif
	...
	if (issqlrelay) {
		...
		// msodbcsql18 reports the ODBC 2.x calls as unsupported
	...
	if (issqlrelay) {
		...
		// msodbcsql18 implements SQLBrowseConnect
		assertEqualDbc(dbc,(int)supported,(int)SQL_TRUE);
	}
	stdoutput.printf("\n");
	...
	if (issqlrelay) {
		...
		// msodbcsql18 implements SQLDescribeParam
		assertEqualDbc(dbc,(int)supported,(int)SQL_TRUE);
	}
	stdoutput.printf("\n");
	...
	if (issqlrelay) {
		...
		// msodbcsql18 implements SQLBulkOperations
		assertEqualDbc(dbc,(int)supported,(int)SQL_TRUE);
	}
	stdoutput.printf("\n");
	#endif
	...
	// mssql supports all four isolation levels (both directly and
	// through SQL Relay); verify each set with a get.
	//
	// through SQL Relay the get fails.  odbcconnection has no
	// getIsolationLevelQuery() override, so
	// sqlrserverconnection::getIsolationLevel()
	// (src/server/sqlrserverconnection.cpp:714) short-circuits and
	// returns the literal "unknown", which SQLR_GetIsolationLevel()
	// (src/api/odbc/sqlrodbc.cpp:2445) can't map to an SQL_TXN_* value,
	// so SQLGetConnectAttr(SQL_ATTR_TXN_ISOLATION) returns SQL_ERROR.
	// pinned rather than dropped, so that adding the override to
	// odbcconnection trips this test instead of going unnoticed.
	erg=SQLSetConnectAttr(dbc,SQL_ATTR_TXN_ISOLATION,
	...
	if (issqlrelay) {
		...
		// the native driver supports cursor sensitivity; get and
		// set succeed
		erg=SQLGetStmtAttr(stmt,SQL_ATTR_CURSOR_SENSITIVITY,
				(SQLPOINTER)&stmtinitial,0,&stmtstrlen);
		assertSuccessStmt(stmt,erg);
	...
	erg=SQLGetStmtAttr(stmt,SQL_ATTR_QUERY_TIMEOUT,
			(SQLPOINTER)&stmtinitial,0,&stmtstrlen);
	assertSuccessStmt(stmt,erg);
	// both arms default to no timeout
	...
	if (issqlrelay) {
		...
		// msodbcsql18 rounds the requested 4096 up to the 8000-byte
		// maximum length of a non-lob column
		assertEqualStmt(stmt,(int)stmtulenval,8000);
	}
	erg=SQLSetStmtAttr(stmt,SQL_ATTR_MAX_LENGTH,
			(SQLPOINTER)(uintptr_t)stmtinitial,0);
	assertSuccessStmt(stmt,erg);
	...
	if (issqlrelay) {
		...
		// msodbcsql18 does support statement-level async, so the
		// value the app sets is the value that comes back
		erg=SQLGetStmtAttr(stmt,SQL_ATTR_ASYNC_ENABLE,
				(SQLPOINTER)&stmtinitial,0,&stmtstrlen);
		assertSuccessStmt(stmt,erg);
		assertEqualStmt(stmt,(int)stmtinitial,
	...
	// cursors); the native driver doesn't implement it and raises HYC00
	...
	if (issqlrelay) {
		...
		// msodbcsql18 accepts the keyset size set but ignores it
		// for a forward-only cursor, so the get still reports 0
		erg=SQLGetStmtAttr(stmt,SQL_ATTR_KEYSET_SIZE,
				(SQLPOINTER)&stmtinitial,0,&stmtstrlen);
		assertSuccessStmt(stmt,erg);
		assertEqualStmt(stmt,(int)stmtinitial,0);
	...
	if (issqlrelay) {
		...
		// msodbcsql18 accepts the simulate-cursor get and set, and
		// defaults to SQL_SC_UNIQUE
		erg=SQLGetStmtAttr(stmt,SQL_ATTR_SIMULATE_CURSOR,
				(SQLPOINTER)&stmtinitial,0,&stmtstrlen);
		assertSuccessStmt(stmt,erg);
	...
	if (issqlrelay) {
		...
		// the native driver implements SQL_ATTR_METADATA_ID at the
		// statement level
		erg=SQLGetStmtAttr(stmt,SQL_ATTR_METADATA_ID,
				(SQLPOINTER)&stmtinitial,0,&stmtstrlen);
		assertSuccessStmt(stmt,erg);
	...
	if (issqlrelay) {
		...
		// the native driver accepts get and the (no-op) disable of
		// auto-IPD
		erg=SQLGetStmtAttr(stmt,SQL_ATTR_ENABLE_AUTO_IPD,
				(SQLPOINTER)&stmtinitial,0,&stmtstrlen);
		assertSuccessStmt(stmt,erg);
	...
	if (issqlrelay) {
		...
		// the native driver accepts the row-operation-array pointer
		// get and set
		erg=SQLGetStmtAttr(stmt,SQL_ATTR_ROW_OPERATION_PTR,
				(SQLPOINTER)&stmtptrinit,0,&stmtstrlen);
		assertSuccessStmt(stmt,erg);
	...
	if (issqlrelay) {
		...
		// the native driver accepts restoring the NULL initial pointer
		assertSuccessStmt(stmt,erg);
	}
	stdoutput.printf("\n");
	#endif
	...
	// the STATEMENT ATTRIBUTES section above exercised every cursor type;
	// the native driver stays in server-cursor mode afterward and
	// then tries to open a cursor over the following DDL ("the statement
	// used to define the cursor is neither a SELECT nor an EXECUTE"), so
	// re-allocate the statement handle for a clean forward-only state
	SQLFreeHandle(SQL_HANDLE_STMT,stmt);
	erg=SQLAllocHandle(SQL_HANDLE_STMT,dbc,&stmt);
	assertSuccessDbc(dbc,erg);
	// the data sections run in autocommit-on mode; the COMMIT AND
	// ROLLBACK section below turns autocommit off and manages its own
	// transaction
	erg=SQLSetConnectAttr(dbc,SQL_ATTR_AUTOCOMMIT,
		(SQLPOINTER)SQL_AUTOCOMMIT_ON,0);
	assertSuccessDbc(dbc,erg);
	SQLExecDirect(stmt,(SQLCHAR *)"drop table testtable",SQL_NTS);
	// declare the columns null explicitly, so the SQLDescribeCol
	// nullability check below sees SQL_NULLABLE whatever the session's
	// ansi null default is; testchar stays NOT NULL so that check has a
	// SQL_NO_NULLS case too
	...
	// the odbc module's bind format is "?", the same as the native
	// driver's, so both arms use the same query text throughout this
	// test.  the freetds-mssql test has to write "@1, @2, ..." for the
	// SQL Relay arm because freetds binds by name
	...
	SQLLEN		bloblen=10;
	// msodbcsql18 rejects a 0 column size on a SQL_LONGVARCHAR or
	// SQL_LONGVARBINARY parameter with HY104 (invalid precision value);
	// SQL Relay accepts 0.  2147483647 is what SQLDescribeCol reports
	// for the text and image columns
	SQLULEN		longcolsize=(issqlrelay)?0:2147483647;
	...
	// unlike freetds, the odbc module can tell an mssql varchar from
	// a char
	...
	#endif
	// both arms report the full 23/3 (millisecond) precision
	...
	// the odbc module reports the real mssql text/image limit, where
	// freetds reports sqlrelay's configured maxfieldsize
	...
	// disabled for both arms.  the odbc-mssql instance doesn't set
	// mars=yes in its connect string, so the ms sql server driver holds
	// one active statement per connection and re-executing on a second
	// statement handle while the outer select is still streaming fails
	// with "[Microsoft][ODBC Driver 18 for SQL Server]Connection is busy
	// with results for another command".  test/c++/odbc-mssql.cpp works
	// around the same limitation by leaving the outer cursor's
	// setResultSetBufferSize call out, so the whole outer result set is
	// buffered before the nested selects run.  the native msodbcsql18
	// arm hits the same "Connection is busy" error, since it goes
	// through the same connection with the same missing mars=yes.
	#if 0
	SQLFreeStmt(stmt,SQL_CLOSE);
	SQLFreeStmt(stmt,SQL_UNBIND);
	...
	// create commits before the second connection ever reads the table.
	// a table created inside an uncommitted transaction would hold a
	// schema lock that no locking hint can get past
	SQLExecDirect(stmt,(SQLCHAR *)"drop table testtable",SQL_NTS);
	erg=SQLExecDirect(stmt,(SQLCHAR *)
		"create table testtable (testint int null)",
		SQL_NTS);
	assertSuccessStmt(stmt,erg);
	...
	// uncommitted rows until the transaction ends, so the test would hang
	// rather than fail.  readpast skips the locked rows instead, which
	...
	// clean up and disconnect; dbc stays in autocommit-on mode so the
	// drop, and the later sections' DDL, commit as they go
	...
	erg=SQLFetch(stmt);
	assertSuccessStmt(stmt,erg);
	// neither empty lob is really NULL.  mssql stores a zero-length
	// text/image value as a true empty value, and both the odbc module
	// and the native driver read it back as 0 bytes - unlike freetds,
	// which reports the image column as 1 byte, the single 0x00 it
	// encodes for empty blob data.  a genuine NULL does round-trip as
	// SQL_NULL_DATA
	...
	// output parameters work through both the odbc module and
	// msodbcsql18, where the freetds-mssql test has this disabled for
	// both arms - native FreeTDS has no support for stored-procedure
	// output parameters
	SQLFreeStmt(stmt,SQL_CLOSE);
	SQLFreeStmt(stmt,SQL_UNBIND);
	SQLFreeStmt(stmt,SQL_RESET_PARAMS);
	// output parameters come back from a stored procedure invoked via
	...
	// lob output bind
	// the deprecated text and image types can't be output parameters to a
	// stored procedure, and there's no way to select directly into a lob
	// bind variable, so this case is not applicable
	...
	// as with OUTPUT BIND BY POSITION above, this works through both
	// the odbc module and msodbcsql18
	SQLFreeStmt(stmt,SQL_CLOSE);
	SQLFreeStmt(stmt,SQL_UNBIND);
	SQLFreeStmt(stmt,SQL_RESET_PARAMS);
	for (int i=0; i<LARGE_BUFFER_LENGTH; i++) {
	...
	largebuffer[LARGE_BUFFER_LENGTH]='\0';
	// a long value comes back through a varchar output parameter of a
	...
	// this keeps freetds-mssql.cpp's insert-and-select-back formulation
	// rather than a {call testproc(?,?)} one, because native FreeTDS has
	// no support for stored-procedure output parameters - no cursor or
	// rpc output binds
	SQLFreeStmt(stmt,SQL_CLOSE);
	SQLFreeStmt(stmt,SQL_UNBIND);
	SQLFreeStmt(stmt,SQL_RESET_PARAMS);
	...
	SQLFreeStmt(stmt,SQL_UNBIND);
	SQLFreeStmt(stmt,SQL_RESET_PARAMS);
	// the parameterized reexecute works through both arms - SQL Relay
	// fakes the bind into the query text, and msodbcsql18 prepares it
	// as a real parameter (native FreeTDS can't, which is why the
	// freetds-mssql test leaves it out)
	...
	assertSuccessStmt(stmt,erg);
	// distinct byte values, one short of a full 256
	...
	erg=SQLTables(stmt,
			...
			(SQLCHAR *)"",SQL_NTS,
			(SQLCHAR *)"",SQL_NTS);
	// both SQL Relay and the native FreeTDS driver implement
	// SQL_ALL_CATALOGS; mssql has catalogs (databases) and the connected
	// database, named after the host, should appear in the list.  through
	// the odbc module SQL Relay lists every database on the server (139
	// of them here), the same as the native driver does, so just look for
	// the connected one - freetds lists only the connected database
	assertSuccessStmt(stmt,erg);
	...
	assertSuccessStmt(stmt,erg);
	// mssql reports schemas, and the test objects land in the connected
	// user's default schema, which is testuser here
	const char	*expschema="testuser";
	bool		schfound=false;
	for (;;) {
		erg=SQLFetch(stmt);
		if (erg==SQL_NO_DATA) {
	...
	// msodbcsql18 takes an empty-string catalog, schema or column
	// filter literally and matches nothing; only a NULL filter means
	// "any".  SQL Relay treats the empty string as "any"
	SQLCHAR		*anyfilter=(issqlrelay)?(SQLCHAR *)"":NULL;
	SQLSMALLINT	anyfilterlen=(issqlrelay)?SQL_NTS:0;
	...
	for (;;) {
		...
		if (!charstring::compare(
				(const char *)tabletype,"SYSTEM TABLE")) {
	...
	// both arms report three table types - SYSTEM TABLE, TABLE, VIEW -
	// where freetds gives a fixed four: ALIAS, SYNONYM, TABLE, VIEW
	assertEqualStmt(stmt,ttrows,3);
	assertTrueStmt(stmt,foundsystemtable);
	...
	erg=SQLExecDirect(stmt,(SQLCHAR *)
		...
		"	testchar char(40))",SQL_NTS);
	assertSuccessStmt(stmt,erg);
	// mssql preserves identifier case, so the tables are lowercase
	erg=SQLTables(stmt,
			anyfilter,anyfilterlen,
			anyfilter,anyfilterlen,
	...
	assertSuccessStmt(stmt,erg);
	// walk through the list and find int, char, varchar, datetime.  the
	// odbc module reports the type names lowercase, the same as the
	// native driver does (freetds uppercases them), so match without
	// regard to case
	SQLCHAR		typname[64];
	SQLLEN		typnameind;
	erg=SQLBindCol(stmt,1,SQL_C_CHAR,
	...
	for (;;) {
		...
		if (!charstring::compareIgnoringCase(
			...
			#if (ODBCVER >= 0x0300)
			assertEqualStmt(stmt,(int)typdatatype,SQL_TYPE_TIMESTAMP);
			#else
			assertEqualStmt(stmt,(int)typdatatype,SQL_TIMESTAMP);
			#endif
			founddatetime=true;
		}
	}
	...
	erg=SQLExecDirect(stmt,(SQLCHAR *)
		...
		SQL_NTS);
	assertSuccessStmt(stmt,erg);
	// the native driver's catalog functions are scoped to the table
	// owner, and mssql puts a table in the creating user's schema, so
	// the native arm has to name testuser.  SQL Relay scopes to the
	// logged-in user on its own, so an empty schema is enough there
	...
	assertSuccessStmt(stmt,erg);
	// SQLColumns reads information_schema.columns, which holds the
	// built-in type names lowercase whatever case the create table used
	...
	for (int c=0; c<6; c++) {
		...
		// #7971 - the catalog (the database) and the schema both
		// come back populated - dev and testuser here
	...
	assertSuccessStmt(stmt,erg);
	// only the key column comes back, so col1 is there and col2 is not
	...
	erg=SQLFetch(stmt);
	assertSuccessStmt(stmt,erg);
	// the constraint is named here, so the reported name is stable;
	// mssql only auto-names an unnamed one, as PK__<table>__<hex>
	...
	assertSuccessStmt(stmt,erg);
	// through the odbc module SQL Relay leads with a SQL_TABLE_STAT row
	// (type=0, no column name), the same as the native driver does, and
	// freetds doesn't, so skip any such row and look for the col1 index
	// row
	...
	for (;;) {
		...
		// both the odbc module and the native driver append the ";1"
		// procedure-group number to the procedure name (freetds
		// doesn't), so match on the leading procedure name
		if (charstring::contains((const char *)procname,"testproc1") ||
			...
			// mssql's sp_stored_procedures reports 2 for a
			// stored procedure, which both arms pass through
			// as SQL_PT_FUNCTION
	...
	assertSuccessStmt(stmt,erg);
	// procedure parameters are named with a leading @, and the native
