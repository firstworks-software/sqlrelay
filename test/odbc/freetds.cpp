...
int main(int argc, char **argv) {
	...
	*dot='\0';
	...
	if (issqlrelay) {
		...
		assertEqualDbc(dbc,(const char *)strval,"ASE");
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	...
	if (issqlrelay) {
		...
		assertEqualDbc(dbc,(int)uintval,39017);
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	...
	erg=SQLGetInfo(dbc,SQL_STRING_FUNCTIONS,
			...
			(SQLSMALLINT)sizeof(uintval),&vallen);
	if (issqlrelay) {
		assertEqualDbc(dbc,(int)uintval,458459);
	} else {
		assertEqualDbc(dbc,(int)uintval,0);
	}
	assertSuccessDbc(dbc,erg);
	...
	if (issqlrelay) {
		...
		assertEqualDbc(dbc,(int)uintval,858);
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	#endif
	...
	if (issqlrelay) {
		...
		assertEqualDbc(dbc,(int)uintval,6);
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	#endif
	...
	if (issqlrelay) {
		...
		// (length 1), so an empty lob round-trips as 1 byte while a
		// genuine NULL round-trips as SQL_NULL_DATA
		...
		// the native FreeTDS driver reads an inserted empty text/image
		// value back as NULL, and reports a NULL text column's length
		// as 16 rather than SQL_NULL_DATA
		assertEqualStmt(stmt,(int)lobclob1ind,(int)SQL_NULL_DATA);
		assertEqualStmt(stmt,(int)lobclob2ind,16);
	...
	SQLFreeStmt(stmt,SQL_UNBIND);
	SQLFreeStmt(stmt,SQL_RESET_PARAMS);
	// FreeTDS rejects a parameter in a bare select ("untyped variable ?
	// is allowed only in a WHERE/SET/VALUES clause"), so the parameterized
	// reexecute is exercised only through SQL Relay
	if (issqlrelay) {
		erg=SQLPrepare(stmt,(SQLCHAR *)"select @1",SQL_NTS);
		assertSuccessStmt(stmt,erg);
	...
	// SQL_ALL_CATALOGS; SAP has catalogs (databases) and the connected
	...
	if (issqlrelay) {
		...
		// the native FreeTDS driver reports an empty primary key name
		assertEqualStmt(stmt,(int)pknameind,0);
	}
	erg=SQLFetch(stmt);
	assertEqualStmt(stmt,(int)erg,(int)SQL_NO_DATA);
	SQLFreeStmt(stmt,SQL_CLOSE);
	...
	for (;;) {
		...
		if (charstring::contains((const char *)procname,"testproc1") ||
			...
			if (issqlrelay) {
				...
				// native FreeTDS driver reports the type as
				// SQL_PT_UNKNOWN
				assertEqualStmt(stmt,(int)proctype,
							SQL_PT_UNKNOWN);
			}
	...
	for (int p=0; p<4; p++) {
		...
		if (issqlrelay) {
			...
			// the native FreeTDS driver reports the parameter mode as
			// unknown and doesn't populate the ordinal position
			assertEqualStmt(stmt,(int)ppmode,
					(int)SQL_PARAM_TYPE_UNKNOWN);
