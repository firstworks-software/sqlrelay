...
// The native FreeTDS ODBC driver returns SQL_NO_DATA from DDL statements
// (create/drop table, create/drop procedure) even though they succeed; SQL
// Relay returns SQL_SUCCESS.
void assertSuccessOrNoDataStmt(SQLHSTMT stmt, SQLRETURN erg) {
	if (issqlrelay) {
	...
	if (erg==SQL_NO_DATA) {
		...
		stdoutput.printf("%s\n",failure);
		stdoutput.printf("expected SQL_NO_DATA, got %d\n",erg);
...
int main(int argc, char **argv) {
	...
	*dot='\0';
	...
	if (issqlrelay) {
		incstr.append(
			"Driver={SQL Relay};"
			"Server=sqlrelay;Port=9032;"
			"Socket=/tmp/freetds-mssql.socket;"
		...
		// on, and tds protocol 7.4 is what mssql 2016 speaks
		incstr.append(
			"Driver={FreeTDS};"
	...
	if (issqlrelay) {
		...
		// FreeTDS doesn't implement SQL_ATTR_METADATA_ID; get and set
		// both raise HY092 (invalid option)
		erg=SQLGetConnectAttr(dbc,SQL_ATTR_METADATA_ID,
				(SQLPOINTER)&dbcinitial,0,&dbcstrlen);
	...
	if (issqlrelay) {
		...
		// FreeTDS doesn't implement SQL_ATTR_ASYNC_ENABLE at the
		// connection level; get and set both raise HY092
		erg=SQLGetConnectAttr(dbc,SQL_ATTR_ASYNC_ENABLE,
				(SQLPOINTER)&dbcinitial,0,&dbcstrlen);
		assertFailureDbc(dbc,erg);
		erg=SQLSetConnectAttr(dbc,SQL_ATTR_ASYNC_ENABLE,
	...
	if (issqlrelay) {
		...
		// FreeTDS doesn't implement SQL_ATTR_AUTO_IPD; raises HY092
		erg=SQLGetConnectAttr(dbc,SQL_ATTR_AUTO_IPD,
				(SQLPOINTER)&dbcuintval,0,&dbcstrlen);
		assertFailureDbc(dbc,erg);
	}
	...
	if (issqlrelay) {
		...
		// the native driver doesn't implement this; get and set
		// raise HYC00
		erg=SQLGetConnectAttr(dbc,SQL_ATTR_TRANSLATE_OPTION,
				(SQLPOINTER)&dbcinitial,0,&dbcstrlen);
		assertFailureDbc(dbc,erg);
	...
	erg=SQLGetInfo(dbc,SQL_DBMS_NAME,
			...
			&vallen);
	if (issqlrelay) {
		assertEqualDbc(dbc,(const char *)strval,"freetds");
	} else {
	...
	if (issqlrelay) {
		...
		assertEqualDbc(dbc,(int)usmallintval,4000);
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	...
	if (issqlrelay) {
		...
		assertEqualDbc(dbc,(int)uintval,131072);
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	...
	if (issqlrelay) {
		...
		// FreeTDS doesn't implement SQL_COLLATION_SEQ
		assertFailureDbc(dbc,erg);
	}
	stdoutput.printf("\n");
	#endif
	...
	if (issqlrelay) {
		...
		assertEqualDbc(dbc,(const char *)strval,"libtdsodbc.so");
	}
	assertSuccessDbc(dbc,erg);
	stdoutput.printf("\n");
	...
	if (issqlrelay) {
		...
		// FreeTDS doesn't implement this SQL_CONVERT_* selector
		assertFailureDbc(dbc,erg);
	}
	stdoutput.printf("\n");
	...
	if (issqlrelay) {
		...
		// the native driver underreports what the backend supports
		assertEqualDbc(dbc,(int)uintval,
			(int)(SQL_SP_EXISTS|
				SQL_SP_ISNOTNULL|
				SQL_SP_ISNULL));
	...
	// freetds doesn't implement this ODBC 3.8 infotype
	...
	// freetds doesn't implement this infotype
	#if (ODBCVER >= 0x0380)
	...
	assertSuccessDbc(dbc,erg);
	// neither SQL Relay nor the native FreeTDS driver implements this
	assertEqualDbc(dbc,(int)supported,(int)SQL_FALSE);
	stdoutput.printf("\n");
	...
	assertSuccessDbc(dbc,erg);
	// neither SQL Relay nor the native FreeTDS driver implements bulk ops
	assertEqualDbc(dbc,(int)supported,(int)SQL_FALSE);
	stdoutput.printf("\n");
	#endif
	...
	if (issqlrelay) {
		...
		// FreeTDS supports cursor sensitivity; get and set succeed
		erg=SQLGetStmtAttr(stmt,SQL_ATTR_CURSOR_SENSITIVITY,
				(SQLPOINTER)&stmtinitial,0,&stmtstrlen);
		assertSuccessStmt(stmt,erg);
		assertEqualStmt(stmt,(int)stmtinitial,(int)SQL_INSENSITIVE);
	...
	if (issqlrelay) {
		...
		// FreeTDS's default query timeout is (SQLULEN)-1
	...
	if (issqlrelay) {
		...
		// FreeTDS accepts the set with info but ignores max rows
	...
	if (issqlrelay) {
		...
		// FreeTDS accepts the set with info but ignores max length
		assertEqualStmt(stmt,(int)stmtulenval,0);
	}
	erg=SQLSetStmtAttr(stmt,SQL_ATTR_MAX_LENGTH,
			(SQLPOINTER)(uintptr_t)stmtinitial,0);
	...
	if (issqlrelay) {
		...
		// the native driver reports async off and rejects any attempt
		// to turn it on
		erg=SQLGetStmtAttr(stmt,SQL_ATTR_ASYNC_ENABLE,
				(SQLPOINTER)&stmtinitial,0,&stmtstrlen);
		assertSuccessStmt(stmt,erg);
		assertEqualStmt(stmt,(int)stmtinitial,
					(int)SQL_ASYNC_ENABLE_OFF);
		// FreeTDS rejects enabling statement-level async
		erg=SQLSetStmtAttr(stmt,SQL_ATTR_ASYNC_ENABLE,
				(SQLPOINTER)(uintptr_t)SQL_ASYNC_ENABLE_ON,0);
		assertFailureStmt(stmt,erg);
	}
	...
	if (issqlrelay) {
		...
		// FreeTDS accepts the keyset size get and set
		erg=SQLGetStmtAttr(stmt,SQL_ATTR_KEYSET_SIZE,
				(SQLPOINTER)&stmtinitial,0,&stmtstrlen);
		assertSuccessStmt(stmt,erg);
		assertEqualStmt(stmt,(int)stmtinitial,0);
	...
	if (issqlrelay) {
		...
		// FreeTDS accepts the simulate-cursor get and set
		erg=SQLGetStmtAttr(stmt,SQL_ATTR_SIMULATE_CURSOR,
				(SQLPOINTER)&stmtinitial,0,&stmtstrlen);
		assertSuccessStmt(stmt,erg);
		assertEqualStmt(stmt,(int)stmtinitial,(int)SQL_SC_NON_UNIQUE);
	...
	if (issqlrelay) {
		...
		// FreeTDS ignores the set; the value stays SQL_RD_ON
	...
	if (issqlrelay) {
		...
		// FreeTDS implements SQL_ATTR_METADATA_ID at the statement level
		erg=SQLGetStmtAttr(stmt,SQL_ATTR_METADATA_ID,
				(SQLPOINTER)&stmtinitial,0,&stmtstrlen);
		assertSuccessStmt(stmt,erg);
		erg=SQLSetStmtAttr(stmt,SQL_ATTR_METADATA_ID,
	...
	if (issqlrelay) {
		...
		// FreeTDS accepts get and the (no-op) disable of auto-IPD
		erg=SQLGetStmtAttr(stmt,SQL_ATTR_ENABLE_AUTO_IPD,
				(SQLPOINTER)&stmtinitial,0,&stmtstrlen);
		assertSuccessStmt(stmt,erg);
		assertEqualStmt(stmt,(int)stmtinitial,(int)SQL_FALSE);
	...
	if (issqlrelay) {
		...
		// FreeTDS accepts the row-operation-array pointer get and set
		erg=SQLGetStmtAttr(stmt,SQL_ATTR_ROW_OPERATION_PTR,
				(SQLPOINTER)&stmtptrinit,0,&stmtstrlen);
		assertSuccessStmt(stmt,erg);
		erg=SQLSetStmtAttr(stmt,SQL_ATTR_ROW_OPERATION_PTR,
	...
	if (issqlrelay) {
		...
		// FreeTDS accepts restoring the NULL initial pointer
		assertSuccessStmt(stmt,erg);
	}
	stdoutput.printf("\n");
	#endif
	...
	// the native FreeTDS driver stays in server-cursor mode afterward and
	...
	if (issqlrelay) {
		// freetds can't tell an mssql varchar from a char, so SQL
		// Relay reports the column type as CHAR
		assertEqualStmt(stmt,(int)datatype,SQL_CHAR);
	} else {
		// the native driver describes the column from the tds
		// metadata rather than from a type name, so it can tell
		// varchar from char
	...
	if (issqlrelay) {
		...
		// driver reports the full 23/3 (millisecond) precision
	...
	// FreeTDS has no cursor support and can't keep more than one result
	// set open on a connection at a time, so fetching the outer select
	// while running a nested select on a second statement handle fails
	// with "Attempt to initiate a new Adaptive Server operation with
	// results pending".  The native FreeTDS ODBC driver has the same
	// limitation, so this section is disabled for both.
	#if 0
	SQLFreeStmt(stmt,SQL_CLOSE);
	SQLFreeStmt(stmt,SQL_UNBIND);
	...
	if (issqlrelay) {
		...
		// mssql stores a zero-length text/image value as a true empty
		// value, and the native driver reads it back as 0 bytes
		assertEqualStmt(stmt,(int)lobclob1ind,0);
		assertEqualStmt(stmt,(int)lobclob2ind,(int)SQL_NULL_DATA);
	...
	// FreeTDS has no support for stored-procedure output parameters -
	// no cursor or rpc output binds - so this uses an
	// insert-and-select-back formulation rather than a
	// {call testproc(?,?)} one
	SQLFreeStmt(stmt,SQL_CLOSE);
	SQLFreeStmt(stmt,SQL_UNBIND);
	SQLFreeStmt(stmt,SQL_RESET_PARAMS);
	...
	SQLFreeStmt(stmt,SQL_UNBIND);
	SQLFreeStmt(stmt,SQL_RESET_PARAMS);
	// the parameterized reexecute runs only through SQL Relay, which
	// fakes the bind into the query text.  the native driver is left
	// out because FreeTDS has no native parameter support here
	if (issqlrelay) {
		erg=SQLPrepare(stmt,(SQLCHAR *)"select @1",SQL_NTS);
		assertSuccessStmt(stmt,erg);
	...
	// database, named after the host, should appear in the list.  SQL
	// Relay lists only the connected database, the native driver lists
	// every database on the server, so just look for the connected one
	assertSuccessStmt(stmt,erg);
	...
	for (;;) {
		...
		if (!charstring::compare(
					(const char *)tabletype,"ALIAS")) {
			foundalias=true;
		} else if (!charstring::compare(
					(const char *)tabletype,"SYNONYM")) {
			foundsynonym=true;
		} else if (!charstring::compare(
	...
	// SQL Relay's mssql table-type list is a fixed four rows - ALIAS,
	// SYNONYM, TABLE, VIEW - while the native FreeTDS driver doesn't
	// implement the SQL_ALL_TABLE_TYPES query and returns an empty
	// result set
	if (issqlrelay) {
		assertEqualStmt(stmt,ttrows,4);
		assertTrueStmt(stmt,foundalias);
	...
	assertSuccessStmt(stmt,erg);
	// walk through the list and find int, char, varchar, datetime.  SQL
	// Relay uppercases the type names, the native driver reports them
	...
	assertSuccessStmt(stmt,erg);
	// SQL Relay returns just the index row, but the native driver
	// leads with a SQL_TABLE_STAT row (type=0, no column name), so
	// skip any such row and look for the col1 index row
	...
	for (;;) {
		...
		// the native driver appends the ";1" procedure-group number
		// to the procedure name, SQL Relay doesn't, so match on the
		// leading procedure name
		if (charstring::contains((const char *)procname,"testproc1") ||
			...
			if (issqlrelay) {
				assertEqualStmt(stmt,(int)proctype,
							SQL_PT_PROCEDURE);
			} else {
				// mssql's sp_stored_procedures reports 2 for a
				// stored procedure, which the native driver
				// passes through as SQL_PT_FUNCTION
				assertEqualStmt(stmt,(int)proctype,
							SQL_PT_FUNCTION);
			}
			proccounter++;
