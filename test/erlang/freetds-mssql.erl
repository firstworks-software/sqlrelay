...
main() ->
    sqlrelay:start(),
    waitForPort(50),
    {ok, _} = sqlrelay:alloc("sqlrelay", 9032, "/tmp/freetds-mssql.socket",
    ...
    sqlrelay:inputBindString("var16", "13:01:01"),
    sqlrelay:inputBindString("var17", "01-Jan-2001 13:01:01"),
    sqlrelay:inputBindString("var18", "junkvalue"),
    sqlrelay:validateBinds(),
    assertTrue(sqlrelay:executeQuery()),
    io:format("~n"),
    ...
    assertEqualsInt(sqlrelay:getColumnLengthByIndex(14), 16),
    assertEqualsInt(sqlrelay:getColumnLengthByName("testdate"), 16),
    assertEqualsInt(sqlrelay:getColumnLengthByIndex(15), 16),
    assertEqualsInt(sqlrelay:getColumnLengthByName("testtime"), 16),
    assertEqualsInt(sqlrelay:getColumnLengthByIndex(16), 16),
    assertEqualsInt(sqlrelay:getColumnLengthByName("testdatetime2"), 16),
    io:format("~n"),
    ...
    assertEqualsInt(sqlrelay:getLongestByName("testdatetime2"), 26),
    io:format("~n"),
    ...
    assertEqualsInt(sqlrelay:getFieldLengthByName(0, "testdatetime2"), 26),
    io:format("~n"),
    ...
    assertEqualsInt(sqlrelay:getFieldLengthByName(7, "testdatetime2"), 26),
    io:format("~n"),
    ...
    sqlrelay:cacheToFile("cachefile1-freetds-mssql"),
    sqlrelay:setCacheTtl(200),
    assertTrue(sqlrelay:sendQuery(
        "select * from testtable order by testint")),
    {ok, Filename1} = sqlrelay:getCacheFileName(),
    assertEqualsString(Filename1, "cachefile1-freetds-mssql"),
    sqlrelay:cacheOff(),
    ...
    assertEqualsString(Filename2, "cachefile1-freetds-mssql"),
    sqlrelay:cacheOff(),
    ...
    sqlrelay:cacheToFile("cachefile2-freetds-mssql"),
    assertTrue(sqlrelay:openCachedResultSet("cachefile1-freetds-mssql")),
    sqlrelay:cacheOff(),
    assertTrue(sqlrelay:openCachedResultSet("cachefile2-freetds-mssql")),
    assertEqualsString(sqlrelay:getFieldByIndex(7, 0), "8"),
    assertEqualsString(sqlrelay:getFieldByIndex(8, 0), null),
    io:format("~n"),
    ...
    assertEqualsString(Filename3, "cachefile1-freetds-mssql"),
    ...
    %% Even if FreeTDS supported cursors, executing a procedure doesn't
    %% declare bind delimiters in the query, so validateBinds can't be
    %% used - disabled in C++

    %% LOB OUTPUT BIND
    %% the deprecated text, ntext and image types can't be output
    %% parameters - disabled in C++
