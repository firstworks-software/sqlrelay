...
class tls extends sqlrtest {
	...
	public static void	main(String[] args) {
		...
		String	cert=
			"../sqlrelay.conf.d/tls/client.pem";
		String	ca=
			"../sqlrelay.conf.d/tls/ca.pem";
		if (System.getProperty("os.name").
			toLowerCase().indexOf("win")>=0) {
			cert=
			"..\\sqlrelay.conf.d\\tls\\client.pfx";
			ca=
			"..\\sqlrelay.conf.d\\tls\\ca.pfx";
		}
		...
		con.enableTls(null,cert,
				null,null,"ca",ca,(short)0);
		...
		assertEquals(cur.getColumnType(0),
						"NUMBER");
		assertEquals(cur.getColumnType(
					"TESTNUMBER"),
						"NUMBER");
		...
		System.out.println(
				"FIELD LENGTHS BY NAME: ");
		assertEquals(cur.getFieldLength(0,
					"TESTNUMBER"),1);
		assertEquals(cur.getFieldLength(0,
					"TESTCHAR"),40);
		assertEquals(cur.getFieldLength(0,
		...
		assertEquals(cur.getFieldLength(0,
					"TESTBLOB"),0);
		System.out.println();
		assertEquals(cur.getFieldLength(7,
					"TESTNUMBER"),1);
		assertEquals(cur.getFieldLength(7,
					"TESTCHAR"),40);
		assertEquals(cur.getFieldLength(7,
		...
		secondcon.enableTls(null,cert,
				null,null,"ca",ca,(short)0);
		assertTrue(secondcur.sendQuery(
			"select count(*) "+
			"from testtable"));
		assertEquals(secondcur.getField(0,0),
						"0");
		assertTrue(con.commit());
		assertTrue(secondcur.sendQuery(
			"select count(*) "+
		...
		StringBuilder	longquery=
					new StringBuilder();
		longquery.append("begin :bindval:='");
		longquery.append(longval.toString());
		longquery.append("'; end;");
		cur.prepareQuery(longquery.toString());
		...
		assertEquals(cur.getField(0,
				"table_type"),"SYNONYM");
		assertEquals(cur.getField(1,
				"table_type"),"TABLE");
		assertEquals(cur.getField(2,
				"table_type"),"VIEW");
		System.out.println();
		...
		System.out.println(
			"TYPE INFO LIST: ");
		assertTrue(cur.getTypeInfoList(
						"number"));
		assertEquals(cur.getColumnName(0),
						"type_name");
		assertEquals(cur.getColumnName(1),
		...
		assertEquals(cur.getField(2,
				"column_name"),
					"TESTVARCHAR");
		assertEquals(cur.getField(3,
				"column_name"),
					"TESTDATE");
		assertEquals(cur.getField(4,
				"column_name"),
					"TESTLONG");
		assertEquals(cur.getField(5,
				"column_name"),
					"TESTCLOB");
		assertEquals(cur.getField(6,
				"column_name"),
					"TESTBLOB");
		assertEquals(cur.getField(0,
				"data_type"),"NUMBER");
		...
		assertTrue(cur.getColumnList(
					"testtable",null));
		assertTrue(cur.getField(0,
			"column_key")!=null &&
			cur.getField(0,
			"column_key").contains("PRI"));
		assertFalse(cur.getField(1,
			"column_key")!=null &&
			cur.getField(1,
			"column_key").contains("PRI"));
		assertTrue(cur.sendQuery(
		...
		assertEquals(cur.getColumnName(12),
					"index_comment");
		assertEquals(cur.rowCount(),1);
		assertEquals(cur.getField(0,"table"),
						"TESTTABLE");
		assertEquals(cur.getField(0,
				"seq_in_index"),"1");
		assertEquals(cur.getField(0,
