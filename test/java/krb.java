...
class krb extends sqlrtest {
	...
	public static void	main(String[] args) {
		...
		for (int il=0;
			il<isolationlevels.length; il++) {
			// oracle requires the isolation
			// level to be the first query of
			// the transaction
		...
		assertEquals(
			cur.getColumnLength("TESTCHAR"),40);
		...
		assertEquals(
			cur.getColumnLength("TESTLONG"),0);
		assertEquals(cur.getColumnLength(5),0);
		assertEquals(
			cur.getColumnLength("TESTCLOB"),0);
		assertEquals(cur.getColumnLength(6),0);
		assertEquals(
			cur.getColumnLength("TESTBLOB"),0);
		System.out.println();
		...
		assertEquals(
			cur.getLongest("TESTDATE"),9);
		assertEquals(cur.getLongest(4),9);
		assertEquals(
			cur.getLongest("TESTLONG"),9);
		assertEquals(cur.getLongest(5),9);
		assertEquals(
			cur.getLongest("TESTCLOB"),9);
		...
		assertEquals(cur.getField(0,2),
			"testvarchar1");
		assertEquals(cur.getField(0,3),
			"01-JAN-01");
		assertEquals(cur.getField(0,4),
			"testlong1");
		assertEquals(cur.getField(0,5),
			"testclob1");
		...
		assertEquals(cur.getField(7,2),
			"testvarchar8");
		assertEquals(cur.getField(7,3),
			"01-JAN-08");
		assertEquals(cur.getField(7,4),
			"testlong8");
		assertEquals(cur.getField(7,5),
			"testclob8");
		...
		assertTrue(cur.executeQuery());
		numvar=
			cur.getOutputBindInteger("numvar");
		stringvar=cur.getOutputBindString(
						"stringvar");
		floatvar=cur.getOutputBindDouble(
						"floatvar");
		assertEquals(numvar,1);
		assertEquals(stringvar,"hello");
		assertEquals(floatvar,2.5);
		nullvar=cur.getOutputBindString(
						"nullvar");
		assertEquals(nullvar,null);
		cur.getNullsAsEmptyStrings();
		System.out.println();
		...
		assertTrue(cur.executeQuery());
		clobvar=
			cur.getOutputBindClob("clobvar");
		clobvarlength=
			cur.getOutputBindLength("clobvar");
		blobvar=
			cur.getOutputBindBlob("blobvar");
		blobvarlength=
