...
int main(int argc, char **argv) {
	...
	cur->inputBind("9","01/01/2004");
	cur->inputBind("10","04:00:00");
	...
	//assertEquals(cur->getField(0,"testdouble"),"1.5");
	...
	//assertEquals(cur->getField(7,"testdouble"),"8.5");
	...
	cur->cacheToFile("cachefile1-db26");
	cur->setCacheTtl(200);
	assertTrue(cur->sendQuery(
		"select "
		"	* "
	...
	assertEquals(filename,"cachefile1-db26");
	cur->cacheOff();
	...
	cur->cacheToFile("cachefile2-db26");
	assertTrue(cur->openCachedResultSet("cachefile1-db26"));
	cur->cacheOff();
	assertTrue(cur->openCachedResultSet("cachefile2-db26"));
	assertEquals(cur->getField(7,(uint32_t)0),"8");
	assertEquals(cur->getField(8,(uint32_t)0),NULL);
	stdoutput.printf("\n");
...
// #8008
	...
	assertEquals(cur->getOutputBindDouble("7"),1.5);
	...
	// database is schema
	// not super important to test with DB2 6, maybe later
