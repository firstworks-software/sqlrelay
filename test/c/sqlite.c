...
int main(int argc, char **argv) {
	...
	sqlrcur_cacheToFile(cur,"cachefile1-sqlite");
	sqlrcur_setCacheTtl(cur,200);
	assertTrue(sqlrcur_sendQuery(cur,"select * from testtable "
		"order by testint"));
	filename=strdup(sqlrcur_getCacheFileName(cur));
	assertEqStr(filename,"cachefile1-sqlite");
	...
	sqlrcur_cacheToFile(cur,"cachefile2-sqlite");
	assertTrue(sqlrcur_openCachedResultSet(cur,"cachefile1-sqlite"));
	sqlrcur_cacheOff(cur);
	assertTrue(sqlrcur_openCachedResultSet(cur,"cachefile2-sqlite"));
	assertEqStr(sqlrcur_getFieldByIndex(cur,7,0),"8");
	assertEqStr(sqlrcur_getFieldByIndex(cur,8,0),NULL);
	printf("\n");
