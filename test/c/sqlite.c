...
int main(int argc, char **argv) {
	...
	for (il=isolationlevels; *il; il++) {
		...
		assertEqStr(
			sqlrcon_getIsolationLevel(con),*il);
		printf("\n");
	}
	...
	sqlrcur_prepareQuery(cur,
		...
		"	:var6)");
	assertEqInt(
		sqlrcur_countBindVariables(cur),6);
	...
	assertEqInt(
		sqlrcur_getLongestByName(
			cur,"testfloat"),3);
	assertEqInt(
		sqlrcur_getLongestByIndex(cur,2),9);
	assertEqInt(
		sqlrcur_getLongestByName(
			cur,"testchar"),9);
	assertEqInt(
		sqlrcur_getLongestByIndex(cur,3),12);
	assertEqInt(
		sqlrcur_getLongestByName(
			cur,"testvarchar"),12);
	assertEqInt(
	...
	sqlrcur_subDoubles(cur,
		subvars,subvaldoubles,precs,scales);
	assertTrue(sqlrcur_executeQuery(cur));
	assertTrue(sqlrcur_sendQuery(cur,
		"select * from testtable"));
	assertEqStr(
