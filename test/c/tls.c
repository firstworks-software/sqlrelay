...
int main(int argc, char **argv) {
	...
	const char	*bindvals[]={"4", "testchar4",
				...
				"01-JAN-2004",
				"testlong4"};
	const char	*arraybindvars[]={"var1","var2",
					"var3","var4",
					"var5",NULL};
	...
	for (il=isolationlevels; *il; il++) {
		...
		// you can set the isolation level, but to
	...
	assertTrue(sqlrcon_commit(con));
	assertTrue(sqlrcon_setIsolationLevel(con,
					isolationlevels[0]));
	printf("\n");
	...
	sqlrcur_inputBindString(cur,"var2",
					"testchar6");
	sqlrcur_inputBindString(cur,"var3",
					"testvarchar6");
	sqlrcur_inputBindDate(cur,"var4",
			2006,1,1,0,0,0,0,NULL,0);
	sqlrcur_inputBindString(cur,"var5",
					"testlong6");
	sqlrcur_inputBindClob(cur,"var6",
					"testclob6",9);
	sqlrcur_inputBindBlob(cur,"var7",
					"testblob6",9);
	assertTrue(sqlrcur_executeQuery(cur));
	printf("\n");
	...
	sqlrcur_inputBindBlob(cur,"var7",
					"testblob8",9);
	sqlrcur_inputBindString(cur,"var9",
					"junkvalue");
	sqlrcur_validateBinds(cur);
	assertTrue(sqlrcur_executeQuery(cur));
	printf("\n");
	...
	assertTrue(
		sqlrcon_resumeSession(con,port,socket));
	free((char *)socket);
	assertTrue(sqlrcur_resumeCachedResultSet(
						cur,id,filename));
	printf("\n");
	assertEqInt(sqlrcur_firstRowIndex(cur),4);
	assertFalse(sqlrcur_endOfResultSet(cur));
	...
	assertTrue(sqlrcur_sendQuery(cur,
		...
		"		from "
		"			( "
		"		select 1 as testnumber "
		"			from dual "
		"			union "
		"		select 2 as testnumber "
		"			from dual "
		"			union "
		"		select 3 as testnumber "
		"			from dual "
		"			union "
		"		select 4 as testnumber "
		"			from dual "
		"			union "
		"		select 5 as testnumber "
		"			from dual "
		"			union "
		"		select 6 as testnumber "
		"			from dual "
		"			union "
		"		select 7 as testnumber "
		"			from dual "
		"			union "
		"		select 8 as testnumber "
		"			from dual "
		"			) "
		"		where "
		"			testnumber>value; "
