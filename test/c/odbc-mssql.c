...
int main(int argc, char **argv) {
	...
	// arrays of binds do work here - odbc
	// binds them all as strings and mssql
	// converts - but the fixture is already
	// 8 rows without them, so there is
	// nothing left for this section to
	// insert
	...
	// input bind by name
	// odbc binds positionally, with "?", so
	// there is nothing to bind by name.
	// that is a contract rather than a
	// defect: @varN gives "Must declare the
	// scalar variable" and :varN gives
	// "Incorrect syntax near ':'".
	// translatebindvariables="yes" would
	// rewrite the binds, but it also mangles
	// every create procedure below, so it
	// isn't usable here.  the block below is
	// parked rather than deleted, but note
	// that it inserts fixture rows 5 through
	// 7, which the REMAINING FIXTURE ROWS
	// section below already inserts by
	// position - switching this on means
	// taking those out
	#if 0
	...
	// odbc binds positionally, so there is
	// nothing to bind by name.  this inserts
	// fixture row 8, which REMAINING FIXTURE
	// ROWS below already inserts by position
	#if 0
	...
	sqlrcur_inputBindString(cur,"var16","13:01:01");
	sqlrcur_inputBindString(cur,"var17","01-Jan-2001 13:01:01");
	sqlrcur_inputBindString(cur,"var18","junkvalue");
	sqlrcur_validateBinds(cur);
	assertTrue(sqlrcur_executeQuery(cur));
	printf("\n");
	#endif
	...
	// the freetds test puts rows 5 through 8
	// in by name.  they go in by position
	// here instead, so the fixture is still
	// 8 rows and every count and row index
	// below carries over unchanged
	printf("REMAINING FIXTURE ROWS: \n");
	sqlrcur_clearBinds(cur);
	...
	sqlrcur_inputBindDouble(cur,"8",5.00,3,2);
	sqlrcur_inputBindDouble(cur,"9",5.00,3,2);
	sqlrcur_inputBindString(cur,"10","01-Jan-2005 05:00:00");
	sqlrcur_inputBindString(cur,"11","01-Jan-2005 05:00:00");
	sqlrcur_inputBindString(cur,"12","testchar5");
	sqlrcur_inputBindString(cur,"13","testvarchar5");
	sqlrcur_inputBindLong(cur,"14",1);
	sqlrcur_inputBindString(cur,"15","01-Jan-2001");
	sqlrcur_inputBindString(cur,"16","13:01:01");
	sqlrcur_inputBindString(cur,"17","01-Jan-2001 13:01:01");
	...
	sqlrcur_inputBindDouble(cur,"8",6.00,3,2);
	sqlrcur_inputBindDouble(cur,"9",6.00,3,2);
	sqlrcur_inputBindString(cur,"10","01-Jan-2006 06:00:00");
	sqlrcur_inputBindString(cur,"11","01-Jan-2006 06:00:00");
	sqlrcur_inputBindString(cur,"12","testchar6");
	sqlrcur_inputBindString(cur,"13","testvarchar6");
	sqlrcur_inputBindLong(cur,"14",1);
	sqlrcur_inputBindString(cur,"15","01-Jan-2001");
	sqlrcur_inputBindString(cur,"16","13:01:01");
	sqlrcur_inputBindString(cur,"17","01-Jan-2001 13:01:01");
	...
	sqlrcur_inputBindDouble(cur,"4",7.5,2,1);
	sqlrcur_inputBindDouble(cur,"5",7.5,2,1);
	sqlrcur_inputBindDouble(cur,"6",7.5,2,1);
	sqlrcur_inputBindDouble(cur,"7",7.5,2,1);
	sqlrcur_inputBindDouble(cur,"8",7.00,3,2);
	sqlrcur_inputBindDouble(cur,"9",7.00,3,2);
	sqlrcur_inputBindString(cur,"10","01-Jan-2007 07:00:00");
	sqlrcur_inputBindString(cur,"11","01-Jan-2007 07:00:00");
	sqlrcur_inputBindString(cur,"12","testchar7");
	sqlrcur_inputBindString(cur,"13","testvarchar7");
	sqlrcur_inputBindLong(cur,"14",1);
	sqlrcur_inputBindString(cur,"15","01-Jan-2001");
	sqlrcur_inputBindString(cur,"16","13:01:01");
	sqlrcur_inputBindString(cur,"17","01-Jan-2001 13:01:01");
	...
	sqlrcur_inputBindDouble(cur,"8",8.00,3,2);
	sqlrcur_inputBindDouble(cur,"9",8.00,3,2);
	sqlrcur_inputBindString(cur,"10","01-Jan-2008 08:00:00");
	sqlrcur_inputBindString(cur,"11","01-Jan-2008 08:00:00");
	sqlrcur_inputBindString(cur,"12","testchar8");
	sqlrcur_inputBindString(cur,"13","testvarchar8");
	sqlrcur_inputBindLong(cur,"14",1);
	sqlrcur_inputBindString(cur,"15","01-Jan-2001");
	sqlrcur_inputBindString(cur,"16","13:01:01");
	sqlrcur_inputBindString(cur,"17","01-Jan-2001 13:01:01");
	...
	assertEqStr(sqlrcur_getColumnTypeByName(cur,"testdatetime2"),
		"TIMESTAMP");
	printf("\n");
	...
	// odbc reports the ODBC column size - the
	// number of characters it takes to
	// display the value - where freetds
	// reports the storage size in bytes, so
	// every one of these differs from the
	// freetds test
	assertEqInt(sqlrcur_getColumnLengthByIndex(cur,0),10);
	...
	assertEqInt(sqlrcur_getColumnLengthByName(cur,"testtinyint"),3);
	assertEqInt(sqlrcur_getColumnLengthByIndex(cur,3),24);
	assertEqInt(sqlrcur_getColumnLengthByName(cur,"testreal"),24);
	assertEqInt(sqlrcur_getColumnLengthByIndex(cur,4),53);
	assertEqInt(sqlrcur_getColumnLengthByName(cur,"testfloat"),53);
	assertEqInt(sqlrcur_getColumnLengthByIndex(cur,5),4);
	assertEqInt(sqlrcur_getColumnLengthByName(cur,"testdecimal"),4);
	assertEqInt(sqlrcur_getColumnLengthByIndex(cur,6),4);
	assertEqInt(sqlrcur_getColumnLengthByName(cur,"testnumeric"),4);
	assertEqInt(sqlrcur_getColumnLengthByIndex(cur,7),19);
	assertEqInt(sqlrcur_getColumnLengthByName(cur,"testmoney"),19);
	assertEqInt(sqlrcur_getColumnLengthByIndex(cur,8),10);
	assertEqInt(sqlrcur_getColumnLengthByName(cur,"testsmallmoney"),10);
	assertEqInt(sqlrcur_getColumnLengthByIndex(cur,9),23);
	assertEqInt(sqlrcur_getColumnLengthByName(cur,"testdatetime"),23);
	assertEqInt(sqlrcur_getColumnLengthByIndex(cur,10),16);
	assertEqInt(sqlrcur_getColumnLengthByName(cur,"testsmalldatetime"),
		16);
	...
	assertEqInt(sqlrcur_getColumnLengthByIndex(cur,15),16);
	assertEqInt(sqlrcur_getColumnLengthByName(cur,"testtime"),16);
	assertEqInt(sqlrcur_getColumnLengthByIndex(cur,16),27);
	assertEqInt(sqlrcur_getColumnLengthByName(cur,"testdatetime2"),27);
	printf("\n");
	...
	assertMoneyEqLen(sqlrcur_getLongestByIndex(cur,7),6);
	assertMoneyEqLen(sqlrcur_getLongestByName(cur,"testmoney"),6);
	assertMoneyEqLen(sqlrcur_getLongestByIndex(cur,8),6);
	assertMoneyEqLen(sqlrcur_getLongestByName(cur,"testsmallmoney"),6);
	assertEqInt(sqlrcur_getLongestByIndex(cur,9),23);
	assertEqInt(sqlrcur_getLongestByName(cur,"testdatetime"),23);
	...
	assertEqInt(sqlrcur_getLongestByIndex(cur,15),16);
	assertEqInt(sqlrcur_getLongestByName(cur,"testtime"),16);
	assertEqInt(sqlrcur_getLongestByIndex(cur,16),27);
	assertEqInt(sqlrcur_getLongestByName(cur,"testdatetime2"),27);
	printf("\n");
	...
	assertMoneyEqStr(sqlrcur_getFieldByIndex(cur,0,7),"1.0000");
	assertMoneyEqStr(sqlrcur_getFieldByIndex(cur,0,8),"1.0000");
	...
	assertMoneyEqStr(sqlrcur_getFieldByIndex(cur,7,7),"8.0000");
	assertMoneyEqStr(sqlrcur_getFieldByIndex(cur,7,8),"8.0000");
	...
	assertEqStr(sqlrcur_getFieldByIndex(cur,7,14),"2001-01-01");
	...
	assertMoneyEqLen(sqlrcur_getFieldLengthByIndex(cur,0,7),6);
	assertMoneyEqLen(sqlrcur_getFieldLengthByIndex(cur,0,8),6);
	assertEqInt(sqlrcur_getFieldLengthByIndex(cur,0,9),23);
	...
	assertEqInt(sqlrcur_getFieldLengthByIndex(cur,0,15),16);
	assertEqInt(sqlrcur_getFieldLengthByIndex(cur,0,16),27);
	printf("\n");
	...
	assertMoneyEqLen(sqlrcur_getFieldLengthByIndex(cur,7,7),6);
	assertMoneyEqLen(sqlrcur_getFieldLengthByIndex(cur,7,8),6);
	assertEqInt(sqlrcur_getFieldLengthByIndex(cur,7,9),23);
	...
	assertEqInt(sqlrcur_getFieldLengthByIndex(cur,7,15),16);
	assertEqInt(sqlrcur_getFieldLengthByIndex(cur,7,16),27);
	printf("\n");
	...
	assertMoneyEqStr(sqlrcur_getFieldByName(cur,0,"testmoney"),"1.0000");
	assertMoneyEqStr(sqlrcur_getFieldByName(cur,0,"testsmallmoney"),"1.0000");
	...
	assertEqStr(sqlrcur_getFieldByName(cur,0,"testtime"),
		"13:01:01.0000000");
	assertEqStr(sqlrcur_getFieldByName(cur,0,"testdatetime2"),
		"2001-01-01 13:01:01.0000000");
	printf("\n");
	...
	assertMoneyEqStr(sqlrcur_getFieldByName(cur,7,"testmoney"),"8.0000");
	assertMoneyEqStr(sqlrcur_getFieldByName(cur,7,"testsmallmoney"),"8.0000");
	...
	assertEqStr(sqlrcur_getFieldByName(cur,7,"testdate"),"2001-01-01");
	assertEqStr(sqlrcur_getFieldByName(cur,7,"testtime"),
		"13:01:01.0000000");
	assertEqStr(sqlrcur_getFieldByName(cur,7,"testdatetime2"),
		"2001-01-01 13:01:01.0000000");
	printf("\n");
	...
	assertMoneyEqLen(sqlrcur_getFieldLengthByName(cur,0,"testmoney"),6);
	assertMoneyEqLen(sqlrcur_getFieldLengthByName(cur,0,"testsmallmoney"),6);
	assertEqInt(sqlrcur_getFieldLengthByName(cur,0,"testdatetime"),23);
	...
	assertEqInt(sqlrcur_getFieldLengthByName(cur,0,"testtime"),16);
	assertEqInt(sqlrcur_getFieldLengthByName(cur,0,"testdatetime2"),27);
	printf("\n");
	...
	assertMoneyEqLen(sqlrcur_getFieldLengthByName(cur,7,"testmoney"),6);
	assertMoneyEqLen(sqlrcur_getFieldLengthByName(cur,7,"testsmallmoney"),6);
	assertEqInt(sqlrcur_getFieldLengthByName(cur,7,"testdatetime"),23);
	...
	assertEqInt(sqlrcur_getFieldLengthByName(cur,7,"testtime"),16);
	assertEqInt(sqlrcur_getFieldLengthByName(cur,7,"testdatetime2"),27);
	printf("\n");
	...
	// autocommit-off left a transaction
	// open, and switching the transaction
	// model doesn't end it here the way it
	// does under freetds.  the drop below
	// would then sit in that transaction,
	// holding a schema lock that the next
	// section's reader blocks on rather than
	// fails on, so put autocommit back on
	// first
	assertTrue(sqlrcon_autoCommitOn(con));
	assertTrue(sqlrcur_sendQuery(cur,"drop table testtable"));
	printf("\n");
	...
	assertTrue(sqlrcur_executeQuery(cur));
	assertTrue(sqlrcur_sendQuery(cur,"select * from testtable"));
	// the empty clob and the empty blob both
	// come back as true zero-length fields
	// here, where freetds gives the blob the
	// single 0x00 byte its encoder emits.
	// they compare equal to "" because this
	// api compares null-terminated strings,
	// so the lengths are asserted too
	assertEqStr(sqlrcur_getFieldByIndex(cur,0,0),"");
	assertEqInt(sqlrcur_getFieldLengthByIndex(cur,0,0),0);
	assertEqStr(sqlrcur_getFieldByIndex(cur,0,1),NULL);
	assertEqStr(sqlrcur_getFieldByIndex(cur,0,2),"");
	assertEqInt(sqlrcur_getFieldLengthByIndex(cur,0,2),0);
	...
	// the odbc module needs a placeholder
	// for each parameter in the query -
	// "exec testproc" on its own counts 0
	// bind variables and fails to execute.
	// "{call testproc(?,?,?,?,?)}" works too
	...
	assertTrue(sqlrcur_sendQuery(cur,
		...
		"	@out5=null"));
	sqlrcur_prepareQuery(cur,
		"exec testproc @out1,@out2,@out3,@out4,@out5");
	assertEqInt(sqlrcur_countBindVariables(cur),5);
	...
	sqlrcur_getOutputBindDate(cur,"out4",&year,&month,&day,
		&hour,&minute,&second,&microsecond,&tz,&isnegative);
	assertEqInt(numvar,1);
	assertEqStr(stringvar,"hello");
	assertEqDbl(floatvar,2.5);
	...
	// odbc binds positionally, so there is
	// nothing to bind by name.  even if
	// there were, validateBinds() couldn't
	// be used for output binds here.  When
	...
	// long output bind
	// #9406 - defineOutputBindString(name,
	// size) comes back with only (size/2)-1
	// characters
	#if 0
	...
	largebuffer[LARGE_BUFFER_LENGTH]='\0';
	{
		char	q[LARGE_BUFFER_LENGTH+256];
		snprintf(q,sizeof(q),"create procedure testproc "
			"@bindval varchar(%d) ""output as "
			"set @bindval='%s'",LARGE_BUFFER_LENGTH,largebuffer);
		assertTrue(sqlrcur_sendQuery(cur,q));
	}
	sqlrcur_prepareQuery(cur,"exec testproc ?");
	sqlrcur_defineOutputBindString(cur,"1",LARGE_BUFFER_LENGTH);
	assertTrue(sqlrcur_executeQuery(cur));
	assertEqInt(sqlrcur_getOutputBindLength(cur,"1"),
		LARGE_BUFFER_LENGTH);
	assertEqStr(sqlrcur_getOutputBindString(cur,"1"),largebuffer);
	assertTrue(sqlrcur_sendQuery(cur,"drop procedure testproc"));
	printf("\n");
	#endif
	...
	// bind validation
	// odbc binds positionally, and
	// validateBinds() skips bind-by-position
	// variables, so there is nothing to
	// validate
	#if 0
	...
	assertTrue(sqlrcur_sendQuery(cur,"create procedure testproc "
		"@in1 int, ""       @in2 float, ""       @in3 varchar(20), "
	...
	strcpy(query,"insert into testtable ""values (0x");
	qoff=(int)strlen(query);
	for (i=0; i<sizeof(buffer); i++) {
		snprintf(hex,sizeof(hex),"%02x",buffer[i]);
		memcpy(query+qoff,hex,2);
		qoff+=2;
	}
	strcpy(query+qoff,")");
	...
	assertEqStr(sqlrcur_getColumnName(cur,0),"Database");
	// odbc lists INFORMATION_SCHEMA, sys and
	// testuser - the schemas that own an
	// object - rather than every schema, so
	// dbo isn't there
	...
	// the odbc module maps odbc type names,
	// not sql server ones, so "int" and
	// "datetime" both come back "Optional
	// feature not implemented" - INTEGER and
	// TIMESTAMP are the names to ask for.
	// the names it returns are the sql server
	// ones, lowercased
	...
	assertEqStr(sqlrcur_getFieldByName(cur,0,"precision"),"8000");
	...
	// TIMESTAMP comes back as three rows -
	// datetime2, datetime and smalldatetime,
	// in that order - so datetime has to be
	// searched for rather than read out of
	// row 0
	...
	assertEqStr(sqlrcur_getFieldByName(cur,1,"type_name"),"datetime");
	assertEqStr(sqlrcur_getFieldByName(cur,1,"data_type"),"93");
	assertEqStr(sqlrcur_getFieldByName(cur,1,"precision"),"23");
	assertEqStr(sqlrcur_getFieldByName(cur,1,"local_type_name"),
		"datetime");
	printf("\n");
	...
	assertEqStr(sqlrcur_getColumnName(cur,11),"comment");
	assertEqStr(sqlrcur_getColumnName(cur,12),"index_comment");
	// the odbc module emits a leading
	// SQL_TABLE_STAT row - table statistics
	// rather than an index - so the index
	// itself is row 1
	assertEqInt(sqlrcur_rowCount(cur),2);
	assertEqStr(sqlrcur_getFieldByName(cur,0,"table"),"testtable");
	assertEqStr(sqlrcur_getFieldByName(cur,0,"key_name"),"");
	assertEqStr(sqlrcur_getFieldByName(cur,0,"cardinality"),"0");
	assertEqStr(sqlrcur_getFieldByName(cur,0,"index_type"),"0");
	assertEqStr(sqlrcur_getFieldByName(cur,1,"table"),"testtable");
	assertEqStr(sqlrcur_getFieldByName(cur,1,"non_unique"),"0");
	assertEqStr(sqlrcur_getFieldByName(cur,1,"seq_in_index"),"1");
	assertEqStr(sqlrcur_getFieldByName(cur,1,"column_name"),"col1");
	assertEqStr(sqlrcur_getFieldByName(cur,1,"collation"),"A");
	assertEqStr(sqlrcur_getFieldByName(cur,1,"cardinality"),"0");
	assertEqStr(sqlrcur_getFieldByName(cur,1,"index_type"),"1");
	assertTrue(sqlrcur_getFieldByName(cur,1,"key_name")!=NULL&&strlen(
			sqlrcur_getFieldByName(cur,1,"key_name"))>0);
	assertTrue(sqlrcur_sendQuery(cur,"drop table testtable"));
	printf("\n");
	...
	// odbc reports the procedure group
	// number too - mssql lets several
	// procedures share a name, distinguished
	// by the number after the semicolon, and
	// an ungrouped procedure is number 1
