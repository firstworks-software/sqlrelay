...
def main():
	...
	# instantiation
	con=PySQLRClient.sqlrconnection("sqlrelay",9004,"/tmp/sqlitetest.socket",
	...
	haspragmafuncs=False
	if issqlite3:
		version=dbversion.split(".")
		majorversion=int(version[0])
		minorversion=(int(version[1])
				if len(version)>1 and version[1].isdigit()
				else 0)
		haspragmafuncs=(majorversion>3 or
				(majorversion==3 and minorversion>=16))
	output()
	...
	assertTrue(con.commit())
	secondcon=PySQLRClient.sqlrconnection("sqlrelay",9004,"/tmp/sqlitetest.socket",
						"testuser","testpassword",0,1)
