...
def main():
	...
	cur.inputBindDate("11",2005,1,1,5,0,0,0,None,False)
	...
	cur.inputBindDate("11",2006,1,1,6,0,0,0,None,False)
	...
	cur.inputBindDate("11",2007,1,1,7,0,0,0,None,False)
	...
	output("BIND BY POSITION WITH VALIDATION: ")
	cur.clearBinds()
	cur.inputBind("1",8)
	cur.inputBind("2",8)
	cur.inputBind("3",8)
	...
	cur.inputBindDate("11",2008,1,1,8,0,0,0,None,False)
	...
	assertEquals(cur.getColumnType(14),"TEXT")
	assertEquals(cur.getColumnType(15),"TINYTEXT")
	assertEquals(cur.getColumnType(16),"MEDIUMTEXT")
	assertEquals(cur.getColumnType(17),"LONGTEXT")
	...
	# longtext comes back as a string, longblob as bytes, in Python
	assertEquals(cur.getField(0,0),"")
	...
	if majorversion>3:
		...
		buffer=bytes(bytearray(range(256)))
	...
	escapes=[]
	for i in range(255):
		escapes.append(random.randint(0,1))
	buffer=""
	for i in range(255):
		buffer+=ch[choices[i]]
	...
	assertTrue(cur.sendQueryWithLength(query,len(query)))
	assertTrue(cur.sendQuery("select col1 from testtable"))
	assertEquals(cur.getFieldLength(0,0),255)
	assertEqualsBytes(cur.getField(0,0),buffer,255)
	assertTrue(cur.sendQuery("drop table testtable"))
	output()
