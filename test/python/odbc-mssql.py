...
def main():
	...
	# instantiation
	con=PySQLRClient.sqlrconnection("sqlrelay",9007,"/tmp/odbc-mssql.socket",
	...
	secondcon=PySQLRClient.sqlrconnection("sqlrelay",9007,"/tmp/odbc-mssql.socket",
						"testuser","testpassword",0,1)
	...
	# odbc reports the empty clob and the empty blob as true zero-length
	# fields, where freetds gives the blob the single 0x00 byte its
	# encoder emits.  both compare equal to "" either way, so the
	# lengths are asserted too
	assertEquals(cur.getField(0,0),"")
	...
	output("WIDE NCHAR COLUMN: ")
	secondcon=PySQLRClient.sqlrconnection("sqlrelay",9033,
	...
	widencharbuffer='N'*WIDE_NCHAR_LENGTH
	...
	assertEquals(secondcur.getFieldLength(0,"testnchar"),WIDE_NCHAR_LENGTH)
	...
	output("FAILED EXECUTE AFTER OUTPUT BIND DATE: ")
	cur.sendQuery("drop procedure testproc")
	assertTrue(cur.sendQuery(
		"create procedure testproc "
		"	@out1 int output, "
	...
	cur.sendQuery("drop procedure testproc")
	longoutputbindbuffer='C'*LONG_OUTPUT_BIND_LENGTH
	cur.sendQuery(
		"create procedure testproc "
		"@bindval varchar(%d) output as "
		"set @bindval='%s'"%(LONG_OUTPUT_BIND_LENGTH,longoutputbindbuffer))
	cur.prepareQuery("exec testproc ?")
