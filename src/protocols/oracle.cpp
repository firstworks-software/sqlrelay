...
class SQLRSERVER_DLLSPEC sqlrprotocol_oracle : public sqlrprotocol {
	...
	private:
		...
		// the representation a 9i client's own list settled on for
		// each ub1 type, indexed by the type, filled in by
		// countDataTypes9i() and answered by sendDataTypeResponse().
		// 0 means the client offered no representation for that type -
		// either it didn't list the type at all, or it listed it with
		// no conversion type
		byte_t		clientdatatypereps[256];
...
uint16_t sqlrprotocol_oracle::countDataTypes9i(const byte_t *rp,
	...
	for (;;) {
		...
		clientdatatypereps[datatype]=echorep;
...
// the data types a real oracle 10.2 server answers a pre-10g client with,
// captured from the 10.2 server in
// test/protocol/oracle/samples/oracle102-oci7-portable-login-select.cap.
// each row is the type, the type it converts to, and the representation of
// that type, 1 universal or 10 native.  a conversion type of 0 means the
// type is not exchanged, and its row is 2 bytes on the wire rather than 4.
//
// this is the server's own catalog, not the client's.  a real server answers
// from a fixed list of its own, in its own order, and the client's offer only
// picks the representation - the reference server answers types the client
// never offered (0xf6, 0xfd, 0xfe), declines two the client did offer (0x64
// and 0x65, BINARY_FLOAT and BINARY_DOUBLE, which 10g introduced and this
// client can't be handed), and emits 0x28 0x29 0x75 0x78 before 0x22 through
// 0x26 where the client emits them after.  echoing the client's list back,
// which is what this did before, gets all three of those wrong.
//
// the order is the same canonical order ttidatatypes above records: the 102
// rows of that table whose type and conversion type both fit in a ub1 are an
// exact ordered subsequence of these 248 - its first 21 rows are these first
// 21, and its remaining 81 are these last 81.  the 146 rows in between are
// types the 11.2 server's ub2 answer doesn't mention at all, so ttidatatypes
// can't supply them and this list has to stand on its own.
//
// the representation column is only a fallback, for a type the client offered
// no representation for.  every type this list exchanges was in the reference
// client's own offer, so every representation on the wire there came from the
// client - see sendDataTypeResponse()
static const byte_t	ttidatatypes9i[][3]={
	{0x01,0x01,0x01},
	{0x02,0x02,0x0a},
	{0x08,0x08,0x01},
	{0x0c,0x0c,0x0a},
...
bool sqlrprotocol_oracle::sendDataTypeResponse() {
	...
	if (!ub2datatypes && datatypessize) {
		...
		for (uint16_t i=0; i<count; i++) {
			...
			byte_t	convdatatype=ttidatatypes9i[i][1];
			write(&reqpacket,datatype);
			write(&reqpacket,convdatatype);
			if (convdatatype) {
				byte_t	rep=clientdatatypereps[datatype];
				if (!rep) {
					rep=ttidatatypes9i[i][2];
				}
				// a type with no representation at all gets an
				// empty list rather than a 0 representation,
				// which would read as the end of the whole list
				if (rep) {
					write(&reqpacket,rep);
				}
				write(&reqpacket,(byte_t)0);
...
void sqlrprotocol_oracle::putOci7SummaryNative(uint32_t cursorid,
	...
	// rather than anything the answer depends on: it is the same width
	// either way, so nothing downstream of it moves, and the client
	// parses both.  left zero rather than reproduced on a guess at what
	// it means
...
bool sqlrprotocol_oracle::sendErrorPacket(const char *what,
	...
	// only identifiable field is the ora number, as a little endian
	// uint32 right after the prefix.  reproduced byte for byte from a
	// real OCI7 native-mode login failure (redhat9x86 x86 client against
	// the same server); the live pointer value that capture carried
	// (right after the "36 01" marker) is zeroed
...
bool sqlrprotocol_oracle::osql7(const byte_t *rp) {
	...
	// and the running row count, which a fetch answers with and a
	// describe reads as "this cursor is mid-result-set" - a re-parse
	// starts a new result set, so it starts a new count.  query3()
	// pairs these two the same way
...
bool sqlrprotocol_oracle::query2(const byte_t *rp) {
	...
	// sequence number back, the same way osql7()'s does - without this it
	// echoes whatever call ran last, so the answer to the execute claims
	// to be the answer to the parse before it
	callnumber=sequence;
	...
	if (options&(OPTION_DEFINE|OPTION_BIND)) {
		...
		// rather than in a socket that closed under it
	...
	if (options&OPTION_PARSE) {
		...
		// and the running row count - see the note beside the same
		// pair in osql7()
		rowssent[cont->getId(cursor)]=0;
	...
	// already read them.  five real captures - a 9.2.0.4.0
	...
	if (options&OPTION_BIND) {
		...
		// the out and inout captures are byte-identical to each
	...
	// OPTION_COMMIT (bit 8) is not read here.  bits 8 and up of this
	// field are a per-call sequence counter, not independent flags -
	// every real explicit commit seen in any capture arrives as
	// its own separate TTI_COMMIT call, never through this bit, and
	// commit-fetch-3 shows this bit set on a plain
	...
	if (options&OPTION_FETCH) {
		...
		// captures for 1 through 5 columns, and again, at different
		// call-sequence positions, against the shifted-exfet-3
		// captures.
		...
		// counter, not independent flags - shifted-exfet-3's
...
bool sqlrprotocol_oracle::getQuery2Descriptors(const byte_t *rp,
	...
	// every column, which is the shape that segfaulted a real client
	...
	// the portable encoding on this path, and a fresh sweep confirmed
	// the same.  a legacy 32-bit client that
	...
	// to read as a request of its own.
	//
	...
	// in bind order.
	//
	// a pl/sql block sends no values at all: the out and inout
...
bool sqlrprotocol_oracle::sendQuery2Response(sqlrservercursor *cursor) {
	...
	if (nativeencoding) {
		...
		// captured sessions, but none of the 4 correlate with cursor
		...
		// the tail of the same summary object, captured off an 8i
		// server rather than built
		putGenericFooter();
		...
		// re-encoding of the literal above: a three field lead-in and
		// then the same summary object sendOsql7Response() sends, one
		// execution further on.  [0022] of
		// test/protocol/oracle/samples/oracle102-oci7-portable-login-select.cap
		// is 39 bytes where the native half of the same session sends
		// 104, and the native half is what pins the three lead-in
		// fields' widths, the same way it pinned putOci7Summary()'s:
		//
		...
		// native capture, two runs against the same x86 linux 10.2
...
bool sqlrprotocol_oracle::sendLobOperationError(uint32_t oranum,
	...
	if (query3session) {
		...
		putOci7Error(0,0,0,0,oranum,message,
...
bool sqlrprotocol_oracle::sendFetchResponse(sqlrservercursor *cursor,
	...
	// 10.2 captures of the same statement fetched a row at a time show it
	// going 1, 2, 3 across successive answers ([0030], [0034] and [0036]
	// of test/protocol/oracle/samples/
	// 9810-redhat9x86-native-midfetch-defines1-realserver.oraproxy), and
	// oci7.cpp reads it back as cda.rpc and asserts exactly that
	// progression.  the modern fetch path already keeps this count the
...
bool sqlrprotocol_oracle::putField(const char *field,
	...
	switch (columntype) {
		...
		case ORACLE_TYPE_ROWID:
			...
			// samples/9746-dev-oci23api7-native-datatypes-
			...
			if (requestedtype==ORACLE_TYPE_ROWID_DEPRECATED) {
				if (!putRowidField(field,fieldsize)) {
					...
					// a rowid the module couldn't decode
					return false;
				}
				return true;
			}
...
bool sqlrprotocol_oracle::putLobField(sqlrservercursor *cursor, uint32_t col) {
	...
	// is written incrementally below, once a segment actually arrives,
	// so a lob that unexpectedly yields no data can still fall back to
	// a null instead of sending a stray marker with nothing behind it
	...
	for (;;) {
		...
		if (!cont->getLobFieldSegment(cursor,col,
			...
			// start sending - the long-form marker, deferred
			// until the first segment actually arrives (see the
			// comment above)
			if (start) {
				write(&reqpacket,(byte_t)CLR_LONG_FORM_MARKER);
				start=false;
			...
			// re-chunk the segment we just got into wire-sized
			// pieces (a count then the bytes), capped at
			// maxchunk just like putLenBytes() above - a segment
			// read from the lob (up to
			...
			while (suboffset<charsread) {
				uint64_t	subchunk=charsread-suboffset;
				...
				if (bigchunkclr) {
					...
					write(&reqpacket,(byte_t)subchunk);
				}
				reqpacket.append(lobbuffer+suboffset,
							(size_t)subchunk);
				suboffset+=subchunk;
			}
...
void sqlrprotocol_oracle::putOci7Error(uint32_t cursorid,
						byte_t commandtype,
						uint32_t rowsprocessed,
						uint32_t successiterations,
						uint32_t oranum,
	...
	//    9810-redhat9x86-native-midfetch-defines1-realserver.oraproxy (and
	//    [0038] of the -defines3- and -defines1234- captures beside it), the
	//    answer to an ofen past the last row: a 119-byte payload that is
	//    putOci7SummaryNative()'s 93-byte object with 1403 in the error field,
	//    then a ub1 length of 25 and "ORA-01403: no data found\n".  the same
	//    object plus message answers a login failure in packet [0017] of
	//    samples/oracle102-oci7-native-login-wrongpassword.cap, which is what
	//    sendErrorPacket()'s native branch already reproduces.
	//
	//  - the portable half is packet [0017] of samples/
	//    oracle102-oci7-portable-login-wrongpassword.cap: an 82-byte payload
	//    that is putOci7Summary()'s object with 1017 in the same field, then
	//    the same ub1 length and message.  a real oci7 client only ever
	//    negotiates this encoding with this module (see nativeencoding), so
	//    this is the half that actually goes out.
	//
	// a call that succeeded ends at the object with no length byte at all -
	// packet [0026] of the same midfetch capture, the parse's answer, is 93
	...
	if (nativeencoding) {
		putOci7SummaryNative(cursorid,commandtype,
	...
	if (oranum) {
		putLenString(message,messagesize);
	}

	debugStart("oci7 error");
	debugWrite("encoding: %s",(nativeencoding)?"native":"portable");
	debugWrite("cursor id: %d",cursorid);
	debugWrite("rows processed: %d",rowsprocessed);
	debugWrite("error: %u",oranum);
...
bool sqlrprotocol_oracle::sendTransactionError(uint32_t cursorid) {
	...
	if (query3session) {
		...
		putOci7Error(cursorid,0,0,0,oranum,message,messagesize);
	}
...
void sqlrprotocol_oracle::putGenericFooter() {
	...
	// client never negotiates the native encoding here
...
bool sqlrprotocol_oracle::sendQueryError(sqlrservercursor *cursor) {
	...
	// putOci7Error()'s ub1 message length.  the modern path's summary
	// object writes a clr message, which isn't capped that way, but
	// sendQueryError() feeds both paths.
	...
	// older session gets the oci7 one, like a fetch does
	if (query3session) {
...
bool sqlrprotocol_oracle::sendCursorNotOpenError(uint32_t cursorid) {
	...
	if (query3session) {
		...
		// the cursor id the client named goes back even though it
		// names nothing this module has open - it is what the client
		// is waiting to hear about.  no statement ran, so the command
		// type, the row count and the iteration count go out as zero,
		// the way a real server answers a call with no cursor behind
		// it (packet [0017] of either wrongpassword capture)
		putOci7Error(cursorid,0,0,0,ORA_INVALID_CURSOR,
				ORA_INVALID_CURSOR_MESSAGE,
				charstring::getLength(
					ORA_INVALID_CURSOR_MESSAGE));
	}
...
bool sqlrprotocol_oracle::sendUnimplementedFunctionError() {
	...
	if (query3session) {
		...
		putOci7Error(0,0,0,0,ORA_UNIMPLEMENTED_FEATURE,
				ORA_UNIMPLEMENTED_FEATURE_MESSAGE,
				charstring::getLength(
					ORA_UNIMPLEMENTED_FEATURE_MESSAGE));
	}
