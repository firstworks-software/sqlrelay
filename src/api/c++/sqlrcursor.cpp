...
void sqlrcursor::cacheError() {
	...
	pvt->_cachedest->write((uint16_t)DONT_SEND_COLUMN_INFO);
	pvt->_cachedest->write((uint32_t)0);

	// empty output and input/output bind lists
...
void sqlrcursor::getErrorFromServer() {
	...
	// predates protocol version 3 sends no sqlstate at all, and the
...
bool sqlrcursor::getFieldAsDate(uint64_t row, uint32_t col,
	...
	return getFieldAsDate(row,col,false,false,NULL,
				year,month,day,
				hour,minute,second,
...
int16_t sqlrcursor::getFieldAsDateYear(uint64_t row, uint32_t col) {
