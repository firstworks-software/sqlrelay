...
class SQLRConnection : public ObjectWrap {
	...
	private:
		...
		static RET	getBindVariableDelimiterDollarSignSupported(
							const ARGS &args);
		static RET	setQuoteEscapes(const ARGS &args);
		...
		static RET	getCurrentUser(const ARGS &args);
		...
		static RET	getDefaultIsolationLevel(const ARGS &args);
...
void SQLRConnection::Init(Handle<Object> exports) {
	...
	NODE_SET_PROTOTYPE_METHOD(tpl,"getCurrentUser",getCurrentUser);
	...
	NODE_SET_PROTOTYPE_METHOD(tpl,"getDefaultIsolationLevel",getDefaultIsolationLevel);
...
RET SQLRConnection::setQuoteEscapes(const ARGS &args) {

	initLocalScope();

	checkArgCount(args,1);

	sqlrcon(args)->setQuoteEscapes(toString(args[0]));
...
RET SQLRConnection::getCurrentUser(const ARGS &args) {

	initLocalScope();

	checkArgCount(args,0);

	const char	*result=sqlrcon(args)->getCurrentUser();
...
RET SQLRConnection::getDefaultTransactionModel(const ARGS &args) {

	initLocalScope();

	checkArgCount(args,0);

	const char	*result=sqlrcon(args)->getDefaultTransactionModel();

	returnString(result);
}

RET SQLRConnection::setTransactionModel(const ARGS &args) {

	initLocalScope();

	checkArgCount(args,1);

	bool	result=sqlrcon(args)->setTransactionModel(toString(args[0]));

	returnBoolean(result);
}

RET SQLRConnection::getTransactionModel(const ARGS &args) {

	initLocalScope();

	checkArgCount(args,0);

	const char	*result=sqlrcon(args)->getTransactionModel();

	returnString(result);
}

RET SQLRConnection::getDefaultIsolationLevel(const ARGS &args) {

	initLocalScope();

	checkArgCount(args,0);

	const char	*result=sqlrcon(args)->getDefaultIsolationLevel();
