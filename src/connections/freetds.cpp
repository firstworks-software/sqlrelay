	...
	if (sybasedb) {
		...
		if (table && table[0]=='#') {
			if (wild) {
				...
				"		tempdb.information_schema.key_column_usage kcu "
				"	where "
				...
				") ck "
				"on "
				"	c.table_catalog=ck.table_catalog "
				"	and "
				"	c.table_schema=ck.table_schema "
				"	and "
				"	c.table_name=ck.table_name "
				"	and "
				"	c.column_name=ck.column_name "
				"where "
				"	c.table_name like '%s____%%' "
				"	and "
