...
void informixconnection::initDatabaseFeatures() {
	...
	databasefeatures[FEATURE_CATALOG_USAGE]=
		"DATA_MANIPULATION,PROCEDURE_CALLS";
	...
	databasefeatures[FEATURE_RESULT_SET_CONCURRENCIES]=
		"FORWARD_ONLY/READ_ONLY,SCROLL_INSENSITIVE/READ_ONLY,"
	...
	databasefeatures[FEATURE_SQL_KEYWORDS]=
		"after,ansi,append,attach,audit,before,bitmap,"
			"boolean,buffered,byte,cache,call,cluster,clustersize,"
			"codeset,database,datafiles,dataskip,datetime,dba,"
			"dbdate,dbmoney,debug,define,delimiter,deluxe,detach,"
			"dirty,distributions,document,each,elif,exclusive,exit,"
			"explain,express,expression,extend,extent,file,"
			"fillfactor,foreach,format,fraction,fragment,gk,hash,"
			"high,hold,hybrid,if,index,init,labeleq,labelge,"
			"labelgt,labelle,labellt,let,listing,lock,log,low,"
			"matches,maxerrors,medium,mode,modify,money,mounting,"
			"new,nvarchar,off,old,operational,optical,optimization,"
			"page,pdqpriority,pload,private,raise,range,raw,"
			"recordend,recover,referencing,rejectfile,release,"
			"remainder,rename,reserve,resolution,resource,resume,"
			"return,returning,returns,ridlist,robin,rollforward,"
			"round,row,rowids,sameas,samples,schedule,scratch,"
			"serial,share,skall,skinhibit,skshow,smallfloat,"
			"stability,standard,start,static,statistics,stdev,"
			"step,sync,synonym,system,temp,text,timeout,trace,"
			"trigger,units,unlock,variance,wait,while,xload,"
			"xunload";
...
const char *informixconnection::getTableTypeListQuery(
	...
	tabletypelistquery.append(
		"from "
		"	(select "
		"		trim('SYSTEM TABLE') as table_type "
		"	from "
...
static const char	*serialtype=
			...
			"	trim('SERIAL') as local_type_name, "
			"	0 as minimum_scale, "
			"	0 as maximum_scale, "
			"	trim('') as sql_data_type, "
			"	trim('') as sql_datetime_sub, "
...
static const char	*serial8type=
			...
			"	1 as fixed_prec_scale, "
			"	1 as auto_unique_value, "
			"	trim('SERIAL8') as local_type_name, "
			"	0 as minimum_scale, "
			"	0 as maximum_scale, "
			"	trim('') as sql_data_type, "
			"	trim('') as sql_datetime_sub, "
...
static const char	*smallfloattype=
			...
			"	0 as fixed_prec_scale, "
			"	0 as auto_unique_value, "
			"	trim('SMALLFLOAT') as local_type_name, "
			"	0 as minimum_scale, "
			"	0 as maximum_scale, "
			"	trim('') as sql_data_type, "
			"	trim('') as sql_datetime_sub, "
...
static const char	*nvarchartype=
			...
			"	0 as fixed_prec_scale, "
			"	0 as auto_unique_value, "
			"	trim('NVARCHAR') as local_type_name, "
			"	0 as minimum_scale, "
			"	0 as maximum_scale, "
			"	trim('') as sql_data_type, "
			"	trim('') as sql_datetime_sub, "
...
static const char	*lvarchartype=
			...
			"	0 as fixed_prec_scale, "
			"	0 as auto_unique_value, "
			"	trim('LVARCHAR') as local_type_name, "
			"	0 as minimum_scale, "
			"	0 as maximum_scale, "
			"	trim('') as sql_data_type, "
			"	trim('') as sql_datetime_sub, "
...
static const char	*datetimetype=
			...
			"	0 as fixed_prec_scale, "
			"	0 as auto_unique_value, "
			"	trim('DATETIME') as local_type_name, "
			"	0 as minimum_scale, "
			"	0 as maximum_scale, "
			"	trim('') as sql_data_type, "
			"	trim('') as sql_datetime_sub, "
...
static const char	*intervaltype=
			...
			"	0 as fixed_prec_scale, "
			"	0 as auto_unique_value, "
			"	trim('INTERVAL') as local_type_name, "
			"	0 as minimum_scale, "
			"	0 as maximum_scale, "
			"	trim('') as sql_data_type, "
			"	trim('') as sql_datetime_sub, "
...
const char *informixconnection::getColumnListQuery(const char *catalog,
	...
	columnlistquery.append(
		...
		"	trim(decode(mod(cl.coltype,256), "
		"		41, "
...
const char *informixconnection::getKeyAndIndexListQuery(const char *catalog,
	...
	keyandindexlistquery.append(
		...
		"	trim(sc.colname) as column_name, "
		"	trim(case "
		"		when sc.colno=abs(si.part1) and "
						"si.part1<0 then 'D' "
		"		when sc.colno=abs(si.part1) and "
						"si.part1>0 then 'A' "
		"		when sc.colno=abs(si.part2) and "
						"si.part2<0 then 'D' "
		"		when sc.colno=abs(si.part2) and "
						"si.part2>0 then 'A' "
		"		when sc.colno=abs(si.part3) and "
						"si.part3<0 then 'D' "
		"		when sc.colno=abs(si.part3) and "
						"si.part3>0 then 'A' "
		"		when sc.colno=abs(si.part4) and "
						"si.part4<0 then 'D' "
		"		when sc.colno=abs(si.part4) and "
						"si.part4>0 then 'A' "
		"		when sc.colno=abs(si.part5) and "
						"si.part5<0 then 'D' "
		"		when sc.colno=abs(si.part5) and "
						"si.part5>0 then 'A' "
		"		when sc.colno=abs(si.part6) and "
						"si.part6<0 then 'D' "
		"		when sc.colno=abs(si.part6) and "
						"si.part6>0 then 'A' "
		"		when sc.colno=abs(si.part7) and "
						"si.part7<0 then 'D' "
		"		when sc.colno=abs(si.part7) and "
						"si.part7>0 then 'A' "
		"		when sc.colno=abs(si.part8) and "
						"si.part8<0 then 'D' "
		"		when sc.colno=abs(si.part8) and "
						"si.part8>0 then 'A' "
		"		when sc.colno=abs(si.part9) and "
						"si.part9<0 then 'D' "
		"		when sc.colno=abs(si.part9) and "
						"si.part9>0 then 'A' "
		"		when sc.colno=abs(si.part10) and "
						"si.part10<0 then 'D' "
		"		when sc.colno=abs(si.part10) and "
						"si.part10>0 then 'A' "
		"		when sc.colno=abs(si.part11) and "
						"si.part11<0 then 'D' "
		"		when sc.colno=abs(si.part11) and "
						"si.part11>0 then 'A' "
		"		when sc.colno=abs(si.part12) and "
						"si.part12<0 then 'D' "
		"		when sc.colno=abs(si.part12) and "
						"si.part12>0 then 'A' "
		"		when sc.colno=abs(si.part13) and "
						"si.part13<0 then 'D' "
		"		when sc.colno=abs(si.part13) and "
						"si.part13>0 then 'A' "
		"		when sc.colno=abs(si.part14) and "
						"si.part14<0 then 'D' "
		"		when sc.colno=abs(si.part14) and "
						"si.part14>0 then 'A' "
		"		when sc.colno=abs(si.part15) and "
						"si.part15<0 then 'D' "
		"		when sc.colno=abs(si.part15) and "
						"si.part15>0 then 'A' "
		"		when sc.colno=abs(si.part16) and "
						"si.part16<0 then 'D' "
		"		when sc.colno=abs(si.part16) and "
						"si.part16>0 then 'A' "
		"		else null "
...
const char *informixconnection::getProcedureParameterListQuery(
	...
	procedureparameterlistquery.append(
		...
		"	trim('') as data_type, "
		"	trim(case mod(spc.paramtype,256) "
		"		when 0 then 'char' "
		...
		"	trim('') as buffer_length, "
		"	trim('') as decimal_digits, "
		"	10 as num_prec_radix, "
		"	1 as nullable, "
		"	trim('') as remarks, "
		"	trim('') as column_def, "
