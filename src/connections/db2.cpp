...
class SQLRSERVER_DLLSPEC db2cursor : public sqlrservercursor {
	...
	private:
		...
		// diagnostic record is read - db2 clears it on the next
		// call on the handle
		char		sqlstate[6];
...
void db2connection::initDatabaseFeatures() {
	...
	databasefeatures[FEATURE_GRANT_CLAUSES]=
		"WITH_GRANT_OPTION,DELETE_TABLE,INSERT_TABLE,"
			"REFERENCES_TABLE,REFERENCES_COLUMN,SELECT_TABLE,"
	...
	databasefeatures[FEATURE_NUMERIC_FUNCTIONS]=
		"ABS,ACOS,ASIN,ATAN,ATAN2,CEILING,COS,COT,"
			"DEGREES,EXP,FLOOR,LOG,LOG10,MOD,POWER,"
	...
	databasefeatures[FEATURE_REVOKE_CLAUSES]=
		"DELETE_TABLE,INSERT_TABLE,REFERENCES_TABLE,"
			"SELECT_TABLE,UPDATE_TABLE";
	...
	databasefeatures[FEATURE_SQL_KEYWORDS]=
		"AFTER,ALIAS,ALLOW,APPLICATION,ASSOCIATE,"
			"ASUTIME,AUDIT,AUX,AUXILIARY,BEFORE,BINARY,"
			"BUFFERPOOL,CACHE,CALL,CALLED,CAPTURE,"
			"CARDINALITY,CCSID,CLUSTER,COLLECTION,"
			"COLLID,COMMENT,CONCAT,CONDITION,CONTAINS,"
			"COUNT_BIG,CURRENT_LC_CTYPE,CURRENT_PATH,"
			"CURRENT_SERVER,CURRENT_TIMEZONE,CYCLE,"
			"DATA,DATABASE,DAYS,DB2GENERAL,DB2GENRL,"
			"DB2SQL,DBINFO,DEFAULTS,DEFINITION,"
			"DETERMINISTIC,DISALLOW,DO,DSNHATTR,"
			"DSSIZE,DYNAMIC,EACH,EDITPROC,ELSEIF,"
			"ENCODING,END-EXEC1,ERASE,EXCLUDING,EXIT,"
			"FENCED,FIELDPROC,FILE,FINAL,FREE,"
			"FUNCTION,GENERAL,GENERATED,GRAPHIC,"
			"HANDLER,HOLD,HOURS,IF,INCLUDING,"
			"INCREMENT,INDEX,INHERIT,INOUT,INTEGRITY,"
			"ISOBID,ITERATE,JAR,JAVA,LABEL,LC_CTYPE,"
			"LEAVE,LINKTYPE,LOCALE,LOCATOR,LOCATORS,"
			"LOCK,LOCKMAX,LOCKSIZE,LONG,LOOP,MAXVALUE,"
			"MICROSECOND,MICROSECONDS,MINUTES,"
			"MINVALUE,MODE,MODIFIES,MONTHS,NEW,"
			"NEW_TABLE,NOCACHE,NOCYCLE,NODENAME,"
			"NODENUMBER,NOMAXVALUE,NOMINVALUE,NOORDER,"
			"NULLS,NUMPARTS,OBID,OLD,OLD_TABLE,"
			"OPTIMIZATION,OPTIMIZE,OUT,OVERRIDING,"
			"PACKAGE,PARAMETER,PART,PARTITION,PATH,"
			"PIECESIZE,PLAN,PRIQTY,PROGRAM,PSID,"
			"QUERYNO,READS,RECOVERY,REFERENCING,"
			"RELEASE,RENAME,REPEAT,RESET,RESIGNAL,"
			"RESTART,RESULT,RESULT_SET_LOCATOR,RETURN,"
			"RETURNS,ROUTINE,ROW,RRN,RUN,SAVEPOINT,"
			"SCRATCHPAD,SECONDS,SECQTY,SECURITY,"
			"SENSITIVE,SIGNAL,SIMPLE,SOURCE,SPECIFIC,"
			"SQLID,STANDARD,START,STATIC,STAY,"
			"STOGROUP,STORES,STYLE,SUBPAGES,SYNONYM,"
			"SYSFUN,SYSIBM,SYSPROC,SYSTEM,TABLESPACE,"
			"TRIGGER,TYPE,UNDO,UNTIL,VALIDPROC,"
			"VARIABLE,VARIANT,VCAT,VOLUMES,WHILE,WLM,"
			"YEARS";
	...
	databasefeatures[FEATURE_TIME_DATE_FUNCTIONS]=
		"DAYNAME,DAYOFWEEK,DAYOFYEAR,HOUR,MINUTE,"
			"MONTH,MONTHNAME,QUARTER,SECOND,"
			"TIMESTAMPDIFF,WEEK,YEAR,CURDATE,CURTIME,"
			"NOW";
